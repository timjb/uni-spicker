...
% Ergänzung zur letzten Vorlesung, an der passenden Stelle einfügen!
\begin{bem}
...
\begin{defn}
  Ein Morphismus $f : X \to Y$ zwischen simplizialen Mengen heißt \emph{Kan-Faserungen}, falls er die Rechtshochhebeeigenschaft bzgl. $\{\, \Lambda^i[n] \to \Delta[n] \,\}$ hat.
\end{defn}

\begin{defn}
  Eine simpliziale Menge $X$ heißt \emph{Kan-Komplex}, falls $X \to *$ eine Kan-Faserung ist.
\end{defn}

\begin{defn}
  Eine \emph{anodyne Erweiterung} ist ein Morphismus $i : A \to B$ von simplizialen Mengen, welcher die Linkshochhebungseigenschaft welcher die LHHE bzgl. aller Kan-Faserungen hat, \dh{} die Unterkategorie der anodynen Erweiterungen ist die Saturierung von $\{\, \Lambda^i[n] \to \Delta[n] \,\}$, also $\Coff(\{\, \Lambda^i[n] \to \Delta[n] \,\})$.
\end{defn}
...
\begin{defn}
  Eine \emph{triviale Faserung} ist ein Morphismus in $\sSet$, welcher die RHE bzgl. $\{\, \partial \Delta[n] \hookrightarrow \Delta[n] \,\}$, \dh{} bzgl. aller Monomorphismen hat.
\end{defn}
...
\begin{satz}[Gabriel-Zisman]
  Sei $k : Y \to Z$ ein Monomorphismus. Ist dann $i : A \to B$ anodyn, so ist $i \square k : A \times Z \cup_{A \times Y} B \times Y \to B \times Z$ (mit ${\otimes} \coloneqq {\times}$) ebenfalls anodyn.
\end{satz}
...
\begin{defn}
  Morphismen $f, g : X \to Y$ in $\sSet$ heißen \emph{homotop}, falls $f ~ g$ in $Y^X$, \dh{} die Menge der Homotopieklassen von Morphismen ist $\pi_0(Y^X)$.
\end{defn}
...
\begin{kor}[\emph{Homotopieerweiterungseigenschaft, HEE}]\mbox{}\\
  ...
  \begin{centertikzcd}
    Y \times I \arrow[r, "h'"] \arrow[d, hook, "i \times \id_I"] & E \arrow[d, "p"] \\
...
\begin{defn}
  Ein Monomorphismus $i : A \to B$ in $\sSet$ heißt \emph{starker Deformationsretrakt} (SDR), falls ein $r : B \to A$ mit $ri = \id_A$ und $[ir] = [\id_B] \in \pi_0(B^B \text{ in } A/\sSet)$, \dh{} es existiert $h : B \times I \to B$ mit $h_0 = \id_B$, $h_1 = ir$, $h|_{A \times I} = \id_{A \times I}$ oder ein Zigzag solcher $h$'s.
\end{defn}
