...
\begin{defn}
  Eine Unterkategorie $\Lat \subseteq \Cat$ heißt \emph{links-saturiert}, falls $\Lat$ abgeschlossen ist unter Pushouts, transfiniten Kompositionen und Retrakten.
\end{defn}
...
\begin{bsp}
  ...
  Sei $\Lat$ die Unterkat. mit $\Mor(\Lat) \coloneqq \prescript{\boxslash}{} R \coloneqq \Set{i \!\in\! \Mor(\Cat)}{\fa{r \in R\!}\! i \boxslash r}$.
...
\begin{prop}
  Für $L_1 \subseteq L_2 \subseteq \Mor(\Cat)$ gilt $L_1^\boxslash \leq L_2^\boxslash$.
\end{prop}

\begin{defn}
  \begin{itemize}
    \item $L \subseteq \Mor(\Cat)$ heißt \emph{proj. abgeschlossen}, falls $L \supseteq \prescript{\boxslash}{} (L^\boxslash)$.
    \item $R \subseteq \Mor(\Cat)$ heißt \emph{injektiv abgeschlossen}, falls $R \supseteq (\prescript{\boxslash}{} L)^\boxslash$.
  \end{itemize}
\end{defn}

\begin{prop}
  $\prescript{\boxslash}{}(L^\boxslash)$ ist die projektive Hülle von $L$, \dh{} die kleinste Klasse von Morphismen, die projektiv abgeschlossen ist und $L$ umfasst.
\end{prop}
...
\begin{defn}
  Ein faktorisierendes Paar $(L, R)$ heißt \emph{schwaches Faktorisierungssystem} (SFS), falls $L = \prescript{\boxslash}{} R$ und $R = L^\boxslash$.
\end{defn}

\begin{prop}
  Sei $(L, R)$ faktorisierend. Dann ist $(L, R)$ genau dann ein SFS, wenn $L \boxslash R$ und $L$ und $R$ unter Retrakten abgeschlossen sind.
  % L \boxslash R \coloniff \fa{i \in L} \fa{p \in R} i \boxslash p
\end{prop}
...
\begin{defn}
  Ein Tripel $(\Wat, \Cat, \Fat)$ von Unterkategorien einer Kategorie $\MatC$ heißt \emph{Modellstruktur} auf $\MatC$, falls sowohl $(\Cat, \Fat \cap \Wat)$ und $(\Cat \cap \Wat, \Fat)$ schwache Faktorisierungssysteme sind und $\Wat$ die 2-aus-3-Eigenschaft erfüllt.
\end{defn}
...
\begin{bem}
  Wegen $\Cat = \prescript{\boxslash}{} (\Fat \cap \Wat)$ bzw. $\Fat = (\Cat \cap \Wat)^\boxslash$ ist das Datum $(\Wat, \Cat, \Fat)$ überbestimmt.
\end{bem}

\begin{bsp}
  Sei $\MatC$ bivollständig. Sei $\Wat = \{ \, \text{Iso} \, \} = \Cat$. Dann wird $\MatC$ mit $\Fat = \MatC$ eine Modellkategorie.
\end{bsp}
...
\begin{bsp}
  ...
  \begin{centertikzcd}
    ...
    & QX \arrow[ru, twoheadrightarrow, "\sim"]
  \end{centertikzcd}
...
% TODO: Dieses Diagramm muss schöner werden!

\begin{defn}
  Ein \emph{Zylinderobjekt} $X \times I$ zu einem $X \in \Ob(\MatC)$, $\MatC$ Modellkategorie ist ein Objekt zusammen mit Morphismen wie folgt:
  %X \times I \xrightarrow[{\sim}]{p} X
  \begin{centertikzcd}
    X \arrow[rd, "i_0 {\sim}"] \arrow[rrd, "\id"] \\
    & X \times I \arrow[r, "p {\sim}"] & X \\
    X \arrow[ru, "i_1 {\sim}"] \arrow[rru, "\id"]
  \end{centertikzcd}
  Das Zylinderobjekt $X \times I$ heißt \emph{gut}, falls $X \amalg X \to X \times I$ eine Kofaserung ist.
  Ein guter Zylinder heißt \emph{sehr gut}, falls $X \times I \xrightarrow{p} X$ eine azyklische Faserung ist.
\end{defn}

\begin{bsp}
  Sei $\nabla : X \coprod X \to X$ die Kodiagonale. Faktorisieren wir diese wie folgt:
  % TODO: nabla einfügen
  \begin{centertikzcd}
    X \arrow[rd, hook] \\
    & X \amalg X \arrow[r, hook] & X \times I \arrow[r, twoheadrightarrow, "\sim"] & X \\
    X \arrow[ru, hook]
  \end{centertikzcd}
  Dann erhalten wir ein Zylinderobjekt $X \times I$ für $X$. Dieses ist sehr gut, wenn $X$ kofasernd ist.
\end{bsp}

\begin{defn}
  Zwei Morphismen $f, g : X \to Y$ in $\MatC$ heißen \emph{links-homotop} (geschrieben $f \sim^l g$), falls ein Zylinder $X \times I$ und ein Diagramm der Form
  \begin{centertikzcd}
    X \arrow[rd, "\sim"] \arrow[rrd, "f"] \\
    & X \times I \arrow[r, dashed, "h"] & Y \\
    X \arrow[ru, "\sim"] \arrow[rru, "g"]
  \end{centertikzcd}
  existiert. Wir definieren $\pi^l(X, Y) \coloneqq \Hom_\MatC(X, Y) / \langle {\sim^l} \rangle$.
\end{defn}
