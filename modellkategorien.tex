...
\begin{konstr}
  ...
  Die Komposition bildet schwache Äquivalenzen auf Isos ab und induziert daher den gesuchten Funktor $\LD{F} : \Ho(\ModC) \to \HatC$.
  Definiere $\mu : \LD{F} \circ \gamma \to F$ durch $\mu_X \coloneqq F(q : QX \to X)$ für $X \in \Ob(\ModC)$.
  Falls $X$ selbst kofasernd ist, so ist $q$ eine schwache Äquivalenz zwischen kofasernden Objekten und somit $\mu_X = F(q)$ ein Isomorphismen.
\end{konstr}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item $F$ erhält azyklische Faserungen und $U$ azyklische Faserungen.
  \end{itemize}
\end{defn}
...
\begin{kor}
  %Quillenäquivalente Modellkategorien besitzen äquivalente Homotopiekategorien.
...
\begin{prop}
  ...
  \begin{itemize}
    \item $(F, U)$ ist Quillenäquivalenz
    \item $(\LD{F}, \RD{U})$ ist Adjunktion von Äquivalenzen
    % TODO: zwei weitere Bedingungen vom ÜB
  \end{itemize}
\end{prop}

\begin{lem}
  Sei $F \ladj U$ eine Quillenadjunktion.
  Angenommen, $U$ erzeugt schwache Äquivalenzen in $\NodC$.
  Dann sind äquivalent:
  \begin{itemize}
    \item $(F, U)$ ist eine Quillenäquivalenz.
    \item $\eta : X \to UFX$ ist eine schwache Äq. für alle kofasernden $X$.
  \end{itemize}
\end{lem}
