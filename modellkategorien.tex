...
\begin{defn}
  Eine \emph{monoidale Kategorie} ist eine Kategorie $\Cat$ zusammen mit einem Bifunktor $\otimes : \Cat \times \Cat \to \Cat$, einem Objekt $\mathbf{1} \in \Ob(\Cat)$, natürlichen Isomorphismen $\alpha : (\blank \otimes \blank) \circ \blank \Rightarrow \blank \otimes (\blank \circ \blank)$, $\lambda : \mathbf{1} \otimes \blank \Rightarrow \blank$ und $\rho : \blank \otimes \mathbf{1} \Rightarrow \blank$, sodass die Kohärenzdiagramme aus der Definition einer schwachen 2-Kategorie kommutieren.
\end{defn}

\begin{bem}
  Eine monoidale Kategorie ist das gleiche wie eine 2-Kategorie mit einem Objekt.
\end{bem}

\begin{bspe}
  \begin{itemize}
    \item Sei $R$ ein Ring (mit Eins). Dann ist $(R\text{-Bi}\ModC, \otimes_R, R)$ eine monoidale Kategorie.
    \item $(\SetC, \times, \LonelyHeart)$ ist eine monoidale Kategorie.
  \end{itemize}
\end{bspe}

\begin{defn}
  Eine \emph{symmetrische monoidale Kategorie} ist eine monoidale Kategorie zusammen mit einem natürlichen Isomorphismus $\gamma : X \otimes Y \to Y \otimes X$, sodass die geeigneten Kohärenzdiagramme kommutieren. Es reicht aus, zu zeigen, dass folgendes Diagramm kommutiert:
  \begin{centertikzcd}
    (Y \otimes X) \otimes Z \arrow[d, "\alpha"] & (X \otimes Y) \otimes Z \arrow[l, "\gamma"] \arrow[r, "\gamma"] & Z \otimes (X \otimes Y) \arrow[d, "\gamma"] \\
    Y \otimes (X \otimes Z) \arrow[r, "\gamma"] & Y \otimes (Z \otimes X) & (Y \otimes Z) \otimes X \arrow[l, "\alpha"]
  \end{centertikzcd}
  \begin{centertikzcd}
    \mathbf{1} \otimes X \arrow[rr, "\gamma"] \arrow[dr, "\lambda"] && X \otimes \mathbf{1} \arrow[dl, "\rho"] \\
    & X
  \end{centertikzcd}
\end{defn}
...
\begin{defn}
  Ein monoidaler Funktor zwischen (symm.) monoidalen Kategorien $\Cat$ und $\Dat$ ist ein Funktor $F : \Cat \to \Dat$ zusammen mit natürlichen Isomorphismen $F \blank \otimes_\Dat F \blank \Rightarrow F (\blank \otimes_\Cat \blank)$ und $F \mathbf{1}_\Cat \Rightarrow \mathbf{1}_\Dat$, welche verträglich mit $\alpha$, $\lambda$, $\rho$ (und evt. $\gamma$) sind.
\end{defn}

\begin{bsp}
  $\SetC \to \RMod, \enspace X \mapsto \text{freier $R$-Modul mit Basis $X$}$
  ist ein monoidaler Funktor.
\end{bsp}

\begin{defn}
  $F, G : \Cat \to \Dat$ monoidale Funktoren. Eine nat. Transformation $\eta : F \Rightarrow G$ lässt \zB{}
  \begin{centertikzcd}
    FX \otimes FY \arrow[r] \arrow[d] & F(X \otimes Y) \arrow[d] \\
    GX \otimes GY \arrow[r] & G(X \otimes Y)
  \end{centertikzcd}
  kommutieren.
\end{defn}

\begin{defn}
  Sei $\Cat$ eine monoidale Kategorie. Dann ist ein Rechts-$\Cat$-Modul eine Kategorie $\Dat$ mit $\otimes : \Dat \times \Cat \to \Dat$ und \ldots
\end{defn}
...
\begin{bsp}
  Seien $R$, $S$, $T$ drei Ringe, $\Cat \coloneqq \RMod\text{-}S$, $\Dat \coloneqq S\text{-}\ModC\text{-}T$, $\Eat \coloneqq R\text{-}\ModC\text{-}T$,
  \begin{align*}
    & \otimes : \Cat \times \Dat \o \Eat, \quad (M, N) \mapsto M \otimes_S N \\
    & \Hom_r : \Dat^\op \times \Eat \to \Cat, \quad (N, P) \mapsto \Hom_{\ModC\text{-}T}(N, P) \\
    & \Hom_l : \Cat^\op \times \Eat \to \Dat, \quad (M, P) \mapsto \Hom_{\RMod}(M, P)
  \end{align*}
\end{bsp}

\begin{defn}
  Eine monoidale Kategorie $(\Cat, \otimes, \mathbf{1})$ heißt \emph{monoidal abgeschlossen}, wenn $\otimes$ Teil einer Biadjunktion ist.
\end{defn}

\begin{bspe}
  \begin{itemize}
    \item $(R\text{-}\ModC\text{-}R, \otimes_R, R)$
...
\begin{defn}
  Sei $\otimes : \Cat \times \Dat \to \Eat$ eine Biadjunktion, $\Dat$, $\Dat$ und $\Eat$ Modellkategorien.
...
\begin{defn}
  Eine \emph{monoidale Modellkategorie} ist eine Modellkategorie $\ModC$ mit monoidal abgeschlossener Struktur $(M, \otimes, \mathbf{1})$, sodass
  \begin{itemize}
    \item $\otimes : \ModC \times \ModC \to \ModC$ ein Quillenbifunktor und
    \item $Q \mathbf{1} \otimes X \to \mathbf{1} \otimes X \cong X$ und $X \otimes Q \mathbf{1} \to X \otimes \mathbf{1} \cong X$ für alle kofasernden $X$ jeweils schwache Äquivalenzen sind.
  \end{itemize}
\end{defn}

\begin{bem}
  Die zweite Bedingung ist äquivalent zu: $X \cong \Hom_r(\mathbf{1}, X) \to \Hom_r(Q \mathbb{1}, X)$ und $X \cong \Hom_l(\mathbb{1}, X) \to \Hom_l(Q \mathbb{1}), X$ sind schwache Äq. für alle fasernden $X$
\end{bem}

% TODO: Scheibenkategorien als Modellkategorien
% TODO: 2-aus-6-Eigenschaft von Isos, schwachen Äquivalenzen

% TODO: \mathbf{1} ausbessern (zu mathbb), $\Mod$ != $\ModC$ korregieren
