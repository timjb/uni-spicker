...
\begin{satz}[\emph{1. Variationsformel}]
  Sei $\alpha : \vinterval \times \cinterval{a}{b} \to M$ eine $\mathcal{C}^\infty$-Variation von einer $\mathcal{C}^\infty$-Kurve $c = \alpha_0 : \cinterval{a}{b} \to M$. Sei $\norm{\dot{c}(t)} = \text{konst} \not= 0$. Dann gilt mit $X(t) \coloneqq \tfrac{\partial \alpha}{\partial s} (0, t)$ (\emph{Variationsvektorfeld}):
  \[ \frac{\d}{\d s} |_{s=0} L(\alpha_s) = \tfrac{1}{\norm{\dot{c}(t)}} \left( g(X, \dot{c})|_a^b - \Int{a}{b}{g(X(\tau), \tfrac{D \dot{c}}{\d t})}{\tau} \right) \]
\end{satz}

% Satz 13.2bis

\begin{satz}[1. Variationsformel für stückweise glattes $c$]
