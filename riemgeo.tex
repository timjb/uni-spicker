...
\newcommand{\KF}{KF} % Killing-Fields
...
\begin{defn}
  Eine \emph{Wirkung} einer eine Lie-Gruppe $G$ auf einer diff'bar Mft $M$ ist ein Gruppenhomomorphismus $\phi : G \to \Diff(M)$, sodass $G \times M \to M, \enspace (g, m) \mapsto \phi(g)(m)$ glatt ist.
\end{defn}
...
% 24.3
\begin{lem}
  Jede 1-Param-UG $\phi : \R \to G$ definiert ein links-inv. VF $X \in \chi(G)$, dessen Integralkurve durch $e$ gerade $\phi$ ist: $c_e = \phi$.
\end{lem}

\begin{fazit}
  $\fa{x \in T_e G} \exu{\text{1-Param-UG $\phi_x$ mit $\dot{\phi_x}(0) = x$}}$
\end{fazit}
...
\begin{defn}
  Sei $(M, g)$ eine Riem. Mft. Ein VF $X \in \chi(M)$ heißt \emph{Killing-Feld}, wenn die lokalen Flüsse $\Phi_t$ von $X$ aus lokalen Isometrien bestehen, \dh{}
  \[
...
% 24.5
\begin{satz}
  Sei $P$ ein sym. Raum, $G \coloneqq \Iso(P)$, $\Gie \cong T_e G$ die Lie-Algebra von $G$. Dann ist die Abbildung
  \[
...
\begin{defn}
  ...
  \begin{align*}
    ...
    \mathcal{P}_p \coloneqq \iota^{-1}(\Set{X \in \KF(P)}{\nabla X(p) = 0}).
  \end{align*}
\end{defn}

% Ziel: $\Gie = k_p \oplus \mathcal{F}_p$ als direkte Summe von UVRen

% 24.6
\begin{lem}
  Sei $P$ ein symmetrischer Raum, $p \in P$. Dann gilt
  \[ \fa{v \in T_p P} \exu{\tilde{v} \in \mathcal{P}_p} \fa{s \in \R} \exp(s \tilde{v}) = t^{\gamma_v}(s). \]
\end{lem}
