...
\begin{lem}
  Seien $X, Y \in \chi(M)$ mit $\fa{f \in \Cont^\infty(M)} X(f) = Y(f)$. Dann gilt $X \equiv Y$.
\end{lem}
...
\begin{defn}
  ...
  \[ \Iso(M, g) \coloneqq \Set{ f : M \to M }{ \text{$f$ Isometrie} } \]
\end{defn}
...
% 24.7
\begin{prop}
  $k_p = \mathfrak{G}_p \coloneqq$ Lie-Algebra von $\Iso_p(P)$.
\end{prop}
...
\begin{prop}[\emph{Cartan-Relationen}]\mbox{}\\
  \begin{itemize}
    \miniitem{0.32 \linewidth}{$[k_p, k_p]_\Gie \subseteq k_p$}
    \miniitem{0.32 \linewidth}{$[k_p, p_p]_\Gie \subseteq p_p$}
    \miniitem{0.32 \linewidth}{$[p_p, p_p]_\Gie \subseteq k_p$}
  \end{itemize}
\end{prop}
...
% 24.12
\begin{prop}
  Sei $P$ ein symm. Raum, $p \in P$. Dann ist
  \[
    \tau_p : p_p \to T_p P, \quad
    x \mapsto \iota(x)(p)
  \]
...
\begin{kor}
  Sei $P$ ein symm. Raum. Dann ist
  \[
    R_p(a, b) : T_p P \to T_p P, \quad
    x \mapsto R_p(a, b) x
  \]
  eine Derivation von $R_p$, \dh{} $\forall \, A, B, X, Y, Z \in \chi(P)$ gilt
  \begin{align*}
