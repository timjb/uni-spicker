...
% Ausgelassen: $M \approx \R P^2 \setminus p(A)$, wobei $A \subset S^2$ eine offene Kappe um den Nordpol und $p : S^2 \to \R P^2$ ist. Begründung: $\R P^2 \setminus p(A) \approx S^1 \times \cinterval{-1}{1} / (x, t) \sim (-x, -t)$. Betrachte $\phi : \cinterval{0}{1} \times \cinterval{-1}{1} \to S^1 \times \cinterval{-1}{1} / {\sim}, \quad (x, t) \mapsto (e^{i \pi x}, t)$. Die Abb. $\phi$ faktorisiert über M
...
\begin{bsp}
  \begin{itemize}
    ...
    \item Für $X = \{ p \}$ einpunktig sind Homtopien $H : X \times \cinterval{0}{1} \to Y$ nichts anderes als Wege in $Y$. % Wege können hintereinander durchlaufen werden.
  \end{itemize}
\end{bsp}
...
\begin{defn}
  Zwei stetige Abbildungen $f, g : X \to Y$ heißen \emph{homotop relativ zu} $A \subset X$ (geschrieben $f \simeq g \rel A$), falls es eine Homotopie $H : X \times \cinterval{0}{1} \to Y$ von $f$ nach $g$ gibt mit
  \[
    H(a,t) = H(a,0) \quad
...
\begin{bem}
  Die Einheitsmorphismen sind eindeutig bestimmt, da
...
\begin{defn}
  Ein (kovarianter) \emph{Funktor} $F : \mathcal{C} \to \mathcal{D}$ zw. Kategorien $\mathcal{C}$ und $\mathcal{D}$ besteht aus einer Abb.
  von Objekten $F : \Ob(\mathcal{C}) \to \Ob(\mathcal{D})$ und einem Morphismus
  \[
    F(f) : F(A) \to F(B) \quad
    \text{für jeden Morphismus $f : A \to B$,}
  \]
  sodass gilt:
  \begin{itemize}
...
\begin{bspe}
  \begin{itemize}
    ...
    \[ P(X) \coloneqq \mathcal{A}, \quad P(f : A \to B)(A \subseteq X) \coloneqq f(A) \subseteq B. \]
...
\begin{defn}
  Ein \emph{kontravarianter Funktor} zwischen Kategorien $\mathcal{C}$ und $\mathcal{D}$ ist ein Funktor $F : \mathcal{C}^{\text{op}} \to \mathcal{D}$.
\end{defn}

% Beispiele
% * kontravarianter Potenzmengenfunktor
% * Abbildung \mapsto duale Abbildung
% * C^*-Algebren-Zeugs

\begin{defn}
  Seien $F, G : \mathcal{C} \to \mathcal{D}$ Funktoren. Eine \emph{natürliche Transformation} $\alpha : F \to G$ ordnet jedem $X \in \Ob(\mathcal{C})$ einen Morphismus $\alpha_X \in \Hom_{\mathcal{D}}(F(X), G(X))$ zu, sodass für jeden Morphismus $f : A \to B$ in $\mathcal{C}$ das Diagramm
  % TODO: Natürlichkeits-Diagramm
  kommutiert.
\end{defn}

\begin{bem}
  Ist $H : \mathcal{C} \to \mathcal{D}$ ein weiterer Funktor und $\beta : G \to H$ eine weitere natürliche Transformation, so ist auch $\beta \alpha : F \to H$ definiert durch
  \[ (\beta \alpha)_X \coloneqq \beta_X \circ \alpha_X \]
  eine natürliche Transformation.
\end{bem}

Trafo-Kategorie $\left[F, G\right]$

\begin{bsp}
  kovarianter Funktor $** : Vect_k \to Vect_k$, natürliche Transformation
\end{bsp}
...
\begin{defn}
  ...
  \begin{itemize}
    \item Eine Teilmenge $Y \subset Y$ wird durch eine stetige Abbildung $p : X \to Y$ \emph{gleichmäßig überlagert}, falls es einen diskreten Raum $D$ und einen Homöomorphismus $\phi : p^{-1}(U) \approx U \times D$ gibt, sodass $p = \pi_1 \circ \phi$ gilt, wobei $\pi_1 : X \times D \to X$ die Projektion ist.
...
\begin{bsp}
  $\R \to S^1, \quad t \mapsto e^{2 \pi it}$ ist eine Überlagerung.
\end{bsp}
...
\begin{bem}
  ...
  Falls $d = \text{\#} (p^{-1}(y))$ endlich ist, heißt $p$ eine \emph{$d$-blättrige} Überlagerung. Ansonsten heißt $p$ eine \emph{unendliche Überlagerung}.
\end{bem}
