...
\begin{satz}
  Sei $X$ ein normaler Raum, $F \subset X$ abgeschlossen. Ist $f : F \to \R$ stetig, so existiert eine stetige Fortsetzung $g : X \to \R$ von $f$ auf $X$, d.\,h. $g|_F = f$, für die außerdem gilt:
  \[
...
\begin{bspe}
  \begin{itemize}
    ...
    \item Ein normierter Vektorraum $V$ ist genau dann lokalkompakt, wenn $\dim V < \infty$.
  \end{itemize}
\end{bspe}

\begin{defn}
  Sei $X$ ein Hausdorffraum. Setze $X^+ \coloneqq X \coprod \{ \infty \}$. Eine Menge $U \subset X^+$ heißt offen, wenn
  \begin{itemize}
    \item $U \subset X$ und $U$ ist offen in $X$ oder
    \item $\infty \in U$ und $X \setminus U \subset X$ kompakt ist.
  \end{itemize}
  Dies definiert eine Topologie auf $X^+$, der sogenannten \emph{Einpunktkompaktifizierung} von $X$. Wenn $X$ lokalkompakt ist, dann ist $X^+$ Hausdorffsch. Falls $X$ selbst kompakt ist, so trägt $x+$ die Summentopologie von $X$ und $\{ \infty \}$.
\end{defn}
...
\begin{bem}
  Die Abbildung $f^+$ ist nicht automatisch stetig, z.\,B. nicht für
