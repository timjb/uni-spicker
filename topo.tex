...
\begin{prop}
  %Jede Teilmenge $\mathcal{B} \subset \mathcal{P}(X)$ ist Subbasis von genau einer Topologie $\Tau$ auf $X$.
...
\begin{bem}
  ...
  \end{itemize}
  Dann gibt es eine Kategorie $\mathcal{C}/{\sim}$ mit $\Ob(\mathcal{C}/{\sim}) \coloneqq \Ob(\mathcal{C})$, $\Mor(\mathcal{C}/{\sim}) \coloneqq \Mor(\mathcal{C})/{\sim}$ und $[f] \circ [g] = [f \circ g]$.
\end{bem}

\begin{defn}
  Die \emph{Produktkategorie} $\mathcal{C} \times \mathcal{D}$ von Kat. $\mathcal{C}$ und $\mathcal{D}$ ist def. durch
  \begin{align*}
    \Ob(\mathcal{C} \times \mathcal{D}) & \coloneqq \Ob(\mathcal{C}) \times \Ob(\mathcal{D}), \\
    \Hom_{\mathcal{C} \times \mathcal{D}}((A_1, A_2), (B_1, B_2)) & \coloneqq \Hom_{\mathcal{C}}(A_1, B_1) \times \Hom_{\mathcal{D}}(A_2, B_2).
  \end{align*}
\end{defn}
...
\begin{bspe}
  \begin{itemize}
    ...
    \item Besitzen $\mathcal{C}$ und $\mathcal{D}$ nur je ein Objekt (sind also Gruppen), so ist ein Funktor $F : \mathcal{C} \to \mathcal{D}$ nichts anderes als ein Gruppenhomo.
...
\begin{defn}
  Ein \emph{kontravarianter Funktor} zwischen Kategorien $\mathcal{C}$ und $\mathcal{D}$ ist ein kovarianter Funktor $F : \mathcal{C}^\op \to \mathcal{D}$.
\end{defn}
