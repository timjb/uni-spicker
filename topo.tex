...
\begin{prop}
  Seien $X$ und $Y$ wegzusammenhängend und $x_0 \in X$. Ist $f : X \to Y$ eine Homotopieäquivalenz, dann induziert, dann ist $f_* : \pi_1(X, x_0) \to \pi_1(Y, f(x_0))$ ein Isomorphismus.
\end{prop}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item für je zwei Objekte $A, B \in \Ob(\mathcal{C})$ eine Menge $\Hom(A, B)$ von \emph{Morphismen} von $A$ nach $B$. Für $f \in \Hom(A, B)$ schreibt man auch $f : A \to B$ oder $A \xrightarrow{f} B$. Dabei heißt $\dom(f) \coloneqq A$ \emph{Quelle} (Domain), $\codom{f} \coloneqq B$ \emph{Ziel} (Codomain) von $f$. Die Klasse aller Morphismen wird mit $\Hom(\mathcal{C})$ bezeichnet.
...
\begin{bem}
  Sei $\mathcal{C}$ eine Kategorie, ${\sim}$ eine Äquivalenzrelation auf $\Hom(\mathcal{C})$ für die gilt:
  \begin{itemize}
    \item Falls $f, g \in \Hom(\mathcal{C})$ mit $f \sim g$, so ist $\dom(f) = \dom(g)$ und $\codom(f) = \codom(g)$.
    \item Ist $f \sim g \in \Hom(A, B)$ und $h \sim k \in \Hom(B, C)$, so gilt auch $h \circ f \sim k \circ g \in \Hom(A, C)$.
  \end{itemize}
  Dann gibt es eine Kategorie $\mathcal{C}/{\sim}$ mit $\Ob(\mathcal{C}/{\sim}) \coloneqq \Ob(\mathcal{C})$ und $\Hom(\mathcal{C}/{\sim}) \coloneqq \Hom(\mathcal{C})/{\sim}$.
\end{bem}
...
\begin{defn}
  Ein \emph{Funktor} $F : \mathcal{C} \to \mathcal{D}$ zw. Kategorien $\mathcal{C}$ und $\mathcal{D}$ besteht aus einer Abb.
