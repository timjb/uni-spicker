...
% Ausgelassen: $M \cong \R P^2 \setminus p(A)$, wobei $A \subset S^2$ eine offene Kappe um den Nordpol und $p : S^2 \to \R P^2$ ist. Begründung: $\R P^2 \setminus p(A) \cong S^1 \times \cinterval{-1}{1} / (x, t) \sim (-x, -t)$. Betrachte $\phi : \cinterval{0}{1} \times \cinterval{-1}{1} \to S^1 \times \cinterval{-1}{1} / {\sim}, \quad (x, t) \mapsto (e^{i \pi x}, t)$. Die Abb. $\phi$ faktorisiert über M
...
\begin{prop}
  $\abs{([n], \mathcal{P}([n]) \setminus \{ [n] \})} \cong \partial \Delta^n$.
\end{prop}
...
\begin{prop}
  Sei $K \subset \R^n$ ein kompakter konvexer Körper mit $\inte(K) \not= \emptyset$. Dann gilt $\partial K \cong \partial D^n$ und $K \cong D^n$.
\end{prop}

\begin{kor}
  $\Delta^n \cong D^n$ und $\partial \Delta^n \cong S^{n-1}$.
\end{kor}
