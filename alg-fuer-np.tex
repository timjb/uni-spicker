...
\begin{satz}
  Die Lösung, die der Algorithmus liefert, ist höchstens um den Faktor
  \[ \tfrac{4}{3} - \tfrac{1}{3 p} \]
...
\begin{beweisskizze}
  ...
  \begin{itemize}
    \item Falls $t > \nicefrac{z^*}{3}$, so hat der Algorithmus sogar eine optimale Lösung gefunden.
...
\begin{problem}[\Problem{Maximum Independent Set}, MIS]
  Geg. einen unger. Graphen~$G = (V, E)$, berechne eine \textit{unabhängige Menge}~$M \subseteq V$, \dh{}
  \[
