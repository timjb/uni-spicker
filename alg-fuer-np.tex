...
\begin{satz}
  %Seien $k, n \in \N$
  Gegeben sei ein ungerichteter, zshgder Graph~$G$ mit $n$~Knoten sowie eine kombinatorische Einbettung von~$G$, die den Graphen als $k$-außenplanar darstellt.
  Dann können wir in Zeit~$\O(k n)$ eine Baumzerlegung von~$G$ mit Weite $\leq 3 k - 1$ und $\O(n)$ Knoten berechnen.
\end{satz}
