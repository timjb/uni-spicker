...
\begin{defn}
  ...
  \end{minipage}
  Die in der $j$-ten Ausführung gelöschten Knoten haben \textit{Außenplanaritätslevel~$j$}.
  Der Graph~$G$ heißt \emph{$k$-außenplanar}, falls er eine Einbettung besitzt, bezüglich der jeder Knoten Außenplanaritätslevel $\leq k$ hat.
\end{defn}
...
% 9.32 und 9.33
\begin{defn}
  Sei $T = (V, E_T)$ ein Spannwald eines unger. Graphen $G = (V, E)$.
  \begin{itemize}
    ...
    \item Setze $\mu(G, T) \coloneqq \max_{e \in E} \mu(e)$ und $\nu(G, T) \coloneqq \max_{v \in V} \nu(v)$.
  \end{itemize}
\end{defn}

% 9.34
\begin{lem}
  Sei ein unger. Graph $G = (V, E)$ mit $n$~Knoten. und ein Spannwald $T = (V, E_T)$ von~$G$ gegeben.
  Setze $k \coloneqq \max \{ \mu(G, T) + 1, \nu(G, T) \}$.
...
% 9.40
\begin{lem}
  Gegeben sei ein Graph~$G$ mit $n$ Knoten gegeben sowie eine Baumzerlegung von~$G$ der Weite~$k$ mit $m$~Knoten.
...
\section{Reduktion auf Lineare Programmierung}
...
\begin{alg}[\emph{Randomized Rounding}] \mbox{} \\
  ...
  \begin{itemize}
    ...
    (Eine Lsg des entspr. ILP wäre eine exakte Lösung von MaxSAT. Bemerke: ${\sum}_{C \in \mathcal{C}} x_C \geq z^*$)
    \item
...
Für $S \subseteq V$ sei $\delta(S) \coloneqq \Set{e \in E}{\size{e \cap S} = 1}$ und $g(S) \coloneqq 1$, falls $u \in S$ und $v \in V \setminus S$ mit $r(u) = r(v)$ existieren, und $g(S) \coloneqq 0$ sonst.
...
\[
  \begin{array}{r l l}
    ...
    \text{unter den NBn}
      ...
      & 0 \leq x_e \leq 1 & \text{für alle } e \in E
  \end{array}
\]
