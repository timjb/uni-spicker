...
%   \Pi_j : \Omega = \Omega_1 \times \ldots \times \Omega_n \to \Omega_j
%           (\omega_1, \ldots, \omega_n) \mapsto \omega_j
...
% $\mu(E_1 \times \ldots \times E_n) = \mu_1(E_1) \cdot \ldots \cdot \mu_n(E_n)$ für alle $E_j \in \mathcal{E}_j$.
...
\begin{nota}
  $x_j : U \to \R, \quad (u_1, \ldots, u_n) \mapsto u_j$ % (Projektion auf $j$-te Koordinate)
\end{nota}
...
% Damit ist $(\d x_1(u), \ldots, \d x_j(u))$ eine Dualbasis von $((u, e_1), \ldots, (u, e_n))$
...
% $\{ \d x_{j_1}(u) \wedge \ldots \wedge \d x_{j_k}(u) \mid 1 \leq j_1 < \ldots < j_k \leq n \}$
...
\begin{satz}%[Eigenschaften der äußeren Ableitung]
  \begin{itemize}
    ...
    \begin{multicols}{2}
      ...
      \item $\d (\d \omega) = 0$, falls $\omega$ $\Cont^2$ ist
    \end{multicols}
  \end{itemize}
\end{satz}
...
\begin{lem}
  ...
  %die Abbildungen, welche $U$ mit den "`Deckeln des Zylinders"' $\I \times U \subet V$ identifizieren.
...
% Bemerkung:
% \[ \phi^{-1} : \R^{k{-}1} \to \R^k, \quad (t_1, \ldots, x_{k-1}) \mapsto (0, t_1, \ldots, t_{k-1}) \]
...
\begin{satz}[\emph{Stokes}, klassisch]
...
\begin{satz}[\emph{Cauchyscher Integralsatz}]
