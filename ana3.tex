...
\begin{satz}
  ...
  \begin{itemize}
    ...
    \item $A \subset B \implies \mu(A) \leq \mu(B)$ \hfill (Monotonie)
    \item Aus $A \subset B$ und $\mu(B) < \infty$ folgt $\mu(B \setminus A) = \mu(B) - \mu(A)$
    \item Für $A_1, ..., A_n \in \Ring$ ist $\mu\left(\bigcup_{i = 1}^n A_i \right) \leq \sum_{i = 1}^n \mu(A_i)$ \hfill (Subadditivität)
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \miniitem{0.68 \linewidth}{$A \subset B \implies \mu^*(A) \leq \mu^*(B)$ \hfill (Monotonie)}
...
\begin{satz}
  \begin{itemize}
    ...
    \item $f, g \in \Leb^1(\Omega, \Alg, \mu), \, f \leq g \implies \IntOmu{f} \leq \IntOmu{g}$ \hfill (Monotonie)
    \item $\abs{\IntOmu{f}} \leq \IntOmu{\abs{f}}$ für alle $f \in \Leb^1(\Omega, \Alg, \mu)$ \hfill ($\triangle$-Ungleichung)
  \end{itemize}
\end{satz}
...
\begin{satz}
  ...
  \begin{itemize}
    \item $\phi^*(\lambda \omega_1 + \omega_2) = \lambda \phi^*(\omega_1) + \phi^*(\omega_2)$ \hfill (Linearität)
    \item $\phi^*(\omega \wedge \eta) = (\phi^* \omega) \wedge (\phi^* \eta)$ \hfill (Verträglichkeit mit $\wedge$)
    \item $\psi^*(\phi^* \omega) = (\phi \circ \psi)^* \omega$ \hfill (Funktorialität)
