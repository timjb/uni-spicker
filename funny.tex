...
\begin{lem}
  \begin{itemize}
    \item $P$ ist Projektion auf $Y$ $\iff$ $P : X \to Y$ und $P = \mathrm{Id}$ auf $Y$.
    \item $P : X \to X$ ist Projektion $\implies$ $X = \mathrm{ker}(P) \oplus \mathrm{im}(P)$
    \item $P : X \to X$ ist Projektion $\implies$ $\mathrm{Id} - P$ ist Projektion und $\ker(\mathrm{Id} - P) = \mathrm{im}(P)$, $\mathrm{im}(\mathrm{Id} - P) = \mathrm{ker}(P)$.
...
% Im Paragraph 3 hatten wir für normierte Räume $X$ bereits die Menge der \emph{stetigen (linearen) Projektionen} $\mathcal{P}(X) = \Set{P \in \mathcal{L}(X)}{P^2 = P}$ eingeführt
