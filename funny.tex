...
\begin{satz}[\emph{Rieszscher Darstellungssatz}]
  Sei $X$ ein Hilbertraum,
  \begin{align*}
    J : X \to X', \quad x \mapsto y \mapsto \langle J(x), y \rangle_{X' {\times} X} \coloneqq \scp{y}{x}_X.
  \end{align*}
  Dann ist $J$ ein isometrischer konjugiert linearer Isomorphismus, \dh{} für alle $x, y \in X$ gilt:
  \begin{itemize}
...
\begin{lem}
  Sei $X$ ein Hilbertraum und $A \in \LSO(X)$ koerziv, \dh{}
...
\begin{prob}
  ...
  Es sei $a = (a_{ij})$ gleichmäßig elliptisch, \dh{} es gibt $c_0 > 0$, sodass
...
\begin{samepage}
\begin{satz}[\emph{Hahn-Banach}]
  Sei $X$ ein $\R$-VR und
  \begin{itemize}
    \item $p : X \to \R$ sublinear, \dh{} für alle $x, y \in X$ und $\alpha \in \R_{\geq 0}$ gelte
...
\begin{bem}
  Der Satz kann als Verallgemeinerung des Projektionssatzes für Hilberträume im linearen Fall aufgefasst werden: Ist $X$ Hilbertraum mit abgeschlossenem Unterraum $Y$, so definiere
...
\begin{satz}[\emph{Baire'scher Kategoriensatz}]\mbox{}\\
  Sei $X \not= \emptyset$ ein vollständiger metrischer Raum und $(A_k)_{k \in \N}$ eine Folge abgeschlossener Mengen $A_k \cls X$, sodass $X = \bigcup_{\mathclap{k \in \N}} A_k$. \\
  Dann gibt es $k_0 \in \N$ mit $\inte(A_{k_0}) \not= \emptyset$.
\end{satz}
...
\begin{satz}[\emph{Prinzip der gleichmäßigen Beschränktheit}]\mbox{}\\
...
\begin{satz}[\emph{Banach-Steinhaus}]
...
\begin{satz}[\emph{Banach-Alaoglu}]
...
\begin{satz}[\emph{Eberlein-Shmulyan}]
  Sei $X$ reflexiver Banachraum. Dann ist die abgeschl. Einheitskugel $\overline{B_1(0)} \subset X$ schwach folgenkompakt.
\end{satz}
...
\begin{satz}
  Sei $X$ ein normierter Raum, $M \subset X$ abgeschlossen und konvex. Dann ist $M$ schwach folgenabgeschlossen, \dh{} für jede Folge $(x_k)_{k \in N}$ in $M$ und $x \in X$ gilt:
  \[ x_k \convWeaklyWith{k} x \text{ in } X \enspace \implies \enspace x \in M. \]
\end{satz}

\begin{lem}[\emph{Mazur}]
...
\begin{satz}
  Sei $\Omega \subset \R^n$ offen, beschränkt, zusammenhängend mit $\mathcal{C}^{0,1}$-Rand sowie $\K = \R$. Sei $a_{ij} = a_{ji} \in L^\infty(\Omega)$ für $i,j = 1, \ldots, n$, sodass $a = (a_{ij})$ gleichmäßig elliptisch ist und $f \in L^2(\Omega)$. Betrachte $E \in W^{1,2}(\Omega) \to \R$ gegeben durch
  \[
  ...
  \begin{itemize}
    ...
    \item Die Lösung ist eindeutig, wenn $M$ folgende Eigenschaft besitzt:
...
\begin{lem}
  ...
  \begin{itemize}
    ...
    \[
      \fa{n \not= m} E_n \cap E_m = \{0\}
      \quad \text{und} \quad
      \bigcup_{\mathclap{n \in \N}} \, (E_0 \oplus \ldots \oplus E_n) \text{ dicht in $X$.}
    \]
...
% Aussage (4) bedeutet, dass es zu $x \in X$ und $n \in \N$ f+r $k = 0, \ldots, n$ Zahlen $\alpha_{n,k}$ gibt mit $\norm{x - \sum_{k=0}^n \alpha_{n,k}}_X \convWith{n} 0$
...
\begin{lem}[\emph{Besselsche Ungleichung}]
  ...
  \begin{align*}
    ...
    &= \dist(x, \mathrm{span} \{ e_0, \ldots, e_n \})^2.
  \end{align*}
\end{lem}
...
\begin{satz}[\emph{vom abgeschlossenen Komplement}]
...
\begin{satz}
  Sei $X$ ein normierter Vektorraum, $E$ ein $n$-dimensionaler Unterraum mit Basis $e_1, \ldots, e_n$ und $Y$ ein abgeschlossener Unterraum mit $Y \cap E = \{ 0 \}$. Dann gilt:
  \begin{itemize}
    \item $\ex{e_1', \ldots, e_n' \in X'} e_j'|_Y = 0$ und $\langle e_j', e_i \rangle = \delta_{ij}$.
...
\begin{lem}
  ...
  \begin{itemize}
    \item $P$ ist die orthogonale Projektion auf $\im(P)$, \dh{}
    \[ \fa{x,y \in X} \norm{x-Px} \leq \norm{x - Py}. \]
...
Die Unterräume $E_n = \im(Q_n)$ erfüllen dann (3) aus dem ersten Lemma und (2) mit $X_n = E_0 \oplus \ldots \oplus E_n$.
\fi
...
\begin{satz}[\emph{Spektralsatz} für kompakte Operatoren]\mbox{}\\
  Für $T \in K(X)$ gilt:
  \begin{itemize}
...
\begin{kor}
  Ist $T \in \mathcal{K}$ und $\lambda \in \sigma(T) \setminus \{ 0 \}$, so hat die Resolventenfunktion $\mu \mapsto R(\mu, T)$ in $\lambda$ einen isolierten \emph{Pol} der Ordnung $n_{\lambda}$, \dh{}
...
\begin{kor}[\emph{Fredholmsche Alternative}]
...
\begin{kor}
  Sei $X$ ein endlich-dim. VR über $\C$ und $T : X {\to} X$ linear. Dann gibt es paarwise verschiedene $\lambda_1, \ldots, \lambda_m \in \C$, wobei $1 \leq m \leq \dim X$, sodass $\sigma(T) = \sigma_p(T) = \{ \lambda_1, \ldots, \lambda_m \}$ und Ordnungen $n_{\lambda_j}$ mit $X = \ker \left( (\lambda \Id {-} T)^{n_{\lambda_1}} \right) \oplus \ldots \oplus \ker \left( (\lambda \Id {-} T)^{n_{\lambda_1}} \right)$.
\end{kor}

% Vorlesung vom 6.2.2014

\begin{bem}
  Sei $X$ ein Hilbertraum über $\C$ und $T \in \mathcal{L}(X)$ \emph{normal}, \dh{} $T^* T - T T^* = 0$, wobei $T^* \coloneqq R_{X}^{-1} T' R_X$ mit $R_X$ die Isometrie aus dem Rieszschen Darstellungssatz bzw. $\fa{x, y \in X\!} \!\scp{x}{T^* y} \!=\! \scp{Tx}{y}$, lassen sich die Aussagen noch konkretisieren. Es gilt:
\end{bem}

\begin{satz}[\emph{Spektralsatz} für kompakte normale Operatoren]\mbox{}\\
...
\begin{lem}
  Seien $X$ ein Hilbertraum über $\C$ und $T \in \LSO(x)$ normal.
  \begin{itemize}
    \item  Ist $X \not= 0$, gilt $\sup \sigma(T) \lambda = \norm{T}$
    \item Ist $T$ \emph{selbstadjungiert}, \dh{} $T^* = T$, so ist $\sigma_p(T) \subset \cinterval{-\norm{T}}{\norm{T}}$ und, falls $T$ auch kompakt ist, $\norm{T}$ oder $-\norm{T}$ ein Eigenwert.
    \item Ist $T$ selbstadjungiert und \emph{positiv semidefinit}, \dh{} $\fa{x \in X} \scp{Tx}{x} \geq 0$, so gilt $\sigma_p(T) \subset \cinterval{0}{\norm{T}}$ und falls $T$ auch kompakt ist, ist $\norm{T}$ Eigenwert.
  \end{itemize}
\end{lem}
