...
\begin{bem}
  Sei eine Abbildung $(f, \tilde{f})$ von Faserungen wie folgt gegeben:
  \vspace{-4pt}
  ...
  \end{centertikzcd}
  \vspace{-4pt}
  Dann gibt es ind. Morphismus $f_* : E \to E'$ der zugeh. Serre-SS, der für $r \to \infty$ gegen die Abb. $\tilde{f}_* : H_*(X; G) \to H_*(X'; G)$ konvergiert, welche mit den Filtrierungen $F^p_n$ und $F'^p_n$ verträglich ist. Außerdem entspricht der Morphismus bei $E^2_{p,q}$ der von $B \to B'$ und $F \to F'$ induzierten Abbildung $H_p(B; H_q(F, G)) \to H_p(B'; H_q(F', G))$. \\
  Die Zuordnung $(f, \tilde{f}) \mapsto f_*$ ist funktoriell.
\end{bem}

% Ausgelassen: Bei Faserbündeln: "pathspace construction"
% Ausgelassen: Relative Version
...
% TODO: Transgression
% TODO: "Spectral Sequence Comparison Theorem" und topol. Spezialfall
% TODO: Transgression
