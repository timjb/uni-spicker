...
\begin{prop}
  ...
  \[ \nldots \to H_*(A \cap B) \to H_*(A) \oplus H_*(B) \to H_*(X, A \cap B) \to H_{*-1}(A \cap B) \to \nldots \]
\end{prop}
...
% 5.1
\begin{satz}
  Sei $X$ ein $\Delta$-Komplex und $A \subset X$ ein Unterkomplex. Dann induziert die Inklusion $C_*^\Delta(X, A) \hookrightarrow C_*(X, A)$ Isomorphismen von Homologiegruppen.
\end{satz}
...
\begin{satz}[Ham-Sandwich-Theorem]
  Seien $K_1, \ldots, K_n \subset \R^n$ Lebesgue- messbar und beschränkt. Dann gibt es eine Hyperebene $H \subset \R^n$, die jeden Teilraum genau halbiert.
\end{satz}
...
\begin{prop}[$\Tor$-Rechenregeln]
  ...
  \begin{itemize}
    ...
    \miniitem{0.39 \linewidth}{$\Tor(\ZM{n}) \cong \ker(A \xrightarrow{\cdot n} A)$}
    \item Ist $0 \to A \to B \to C \to 0$ eine \keS{} ab. Gruppen und $H$ eine ab. Gruppe, so erhalten wir eine induzierte exakte Sequenz
    \begin{align*}
