...
%\[ \partial \angles{p_0, \ldots, p_n} \coloneqq \sum_{i=0}^n (-1)^{i} \angles{p_0, \ldots, \hat{p_i}, \ldots, p_n} \]
...
\begin{defn}[\emph{Eilenberg-Steenrod-Axiome}]\mbox{}\\
...
\begin{kor}[Jordanscher Kurvensatz]\mbox{}\\
...
% 6.2 (die Aussage "gerade Abb. haben gerade Abbildungsgrad" wurde in der Übung behandelt)
\begin{prop}
...
% 6.4
\begin{satz}[Ham-Sandwich-Theorem]\mbox{}\\
...
% siehe Hatcher, AT, Appendix "Topology of Cell Complexes"
\begin{bem}
  Sind $X$ und $Y$ lokal kompakte CW-Komplexe, dann ist auch $X \times Y$ ein endlich-dimensionaler CW-Komplex. Ist $X$ oder $Y$ nicht lokal kompakt, so gibt es auch einen Produktkomplex, doch hat dieser eine i.\,A. feinere Topologie als der Produktraum $X \times Y$.
\end{bem}
...
\begin{prop}[$\Ext$-Rechenregeln]
  ...
  \begin{itemize}
    ...
    \miniitem{0.42 \linewidth}{$\Ext(\ZM{n}, G) = G/nG$}
...
\begin{prop}[$\Tor$-Rechenregeln]
  \inlineitem{$\Tor_0(A, B) \cong A \otimes B$} \\
  \inlineitem{$\Tor(A, B) \cong \Tor(B, A)$} \quad
  \inlineitem{$\Tor(\oplus_i A_i, B) \cong \oplus_i \Tor(A_i, B)$} \\
  \inlineitem{$\Tor(A, B) \cong \Tor(T(A), B)$ mit $T(A) < A$ Torsionsuntergruppe} \\
  \inlineitem{$\Tor(A, B) = 0$, falls $A$ torsionsfrei} \quad
  \inlineitem{$\Tor(\ZM{n}, A) \cong \ker(A \xrightarrow{\cdot n} A)$}
  \begin{itemize}
    \item Ist $0 \to A \to B \to C \to 0$ eine \keS{} ab. Gruppen und $H$ eine ab. Gruppe, so erhalten wir eine induzierte exakte Sequenz
    \begin{align*}
      0 & \to \Tor(A, D) \to \Tor(B, D) \to \Tor(C, D) \to \\
      & \to A \otimes D \to B \otimes D \to C \otimes D \to 0.
    \end{align*}
  \end{itemize}
\end{prop}
...
% 12.8
\begin{satz}[\emph{Künneth-Thm} für Homologie]
...
% 12.9
\begin{satz}[\emph{Künneth-Thm} für Kohomologie]
