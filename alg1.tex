...
%\usepackage{extpfeil} % Für diesen Pfeil:
%\newextarrow{\xmapsfrom}{0900}{{\leftarrow\relbar}\relbar\mapstochar}
\makeatletter
...
\begin{defn}
  Falls oben $a_0 \neq 0$ gilt, so ist $\partial f = n$ der \emph{Grad} des Polynoms.
\end{defn}
...
\begin{defn}
  Seien $i, j \in \{ 1, ..., n \}$ mit $i \neq j$. Dann ist die \emph{Transposition} von $i$ und $j$ die Abbildung, die $i$ und $j$ vertauscht, also
  \[
...
\begin{bem}
  Die Verknüpfung von Körperhomomorphismen ist ein Körperhomomorphismus. Falls $f$ bijektiv ist, dann ist auch $f^{-1}$ ein Körperhomomorphismus und $f$ heißt Körperisomorphismus.  Wenn zusätzlich $\K = \K'$ ist, so heißt $f$ Körperautomorphismus.
\end{bem}
...
% Kapitel 2.30. Nicht-Auflösbarkeit bei Grad $\geq 5$

\begin{satz}
  Die Gruppe $A_n$ ist einfach für alle $n \geq 5$.
\end{satz}

% Kapitel 2.31. Der Hauptsatz der Galoistheorie
...
% Satz 2.32. Der Satz von primitiven Element
