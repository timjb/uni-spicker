...
\begin{prop}
  Für alle $t \in \R$ gilt $\abs{e^{it}} = 1$.
\end{prop}

\begin{prop}
  Es gilt für alle $t \in \R$:
  \begin{itemize}
    \miniitem{0.18 \linewidth}{$e^{2\pi} = 1$}
    \miniitem{0.18 \linewidth}{$e^{\pi} = -1$}
    \miniitem{0.28 \linewidth}{$e^{(2\pi + t)i} = e^{ti}$}
    \miniitem{0.32 \linewidth}{$e^{it} = \cos(t) + i \sin(t)$}
  \end{itemize}
\end{prop}

\begin{bem}
  Jede komplexe Zahl $z \in \C$ lässt sich als $z = \abs{z} \cdot e^{is}$ mit $s \in \cointerval{0}{2\pi}$ darstellen.
  Mit $w = \abs{w} \cdot e^t$ gilt $z \cdot w = (\abs{z} \cdot \abs{w}) \cdot e^{i(s+t)}$.
\end{bem}

\begin{defn}
  Für $z = \abs{z} \cdot e^{it} \in \C$ und $n \in \N$ heißen die Zahlen
  \[ \sqrt[n]{\abs{z}} e^{i(t + k 2 \pi)/n} \]
