...
\begin{bem}
  Für $n = 2$ ist eine Möbius-Transformation eine Abb. $z \mapsto \tfrac{az+b}{cz+d}, z \in \C$, sodass Ausdruck def. ist mit $\det \left( \begin{smallmatrix} a & b \\ c & d \end{smallmatrix} \right) \not= 0$.
\end{bem}
...
\begin{defn}
  ...
  \[ \Int{C}{}{f}{\A} \coloneqq \Int{C}{}{f(u) \cdot \sqrt{\det(g_u)}}{\mu (u)} \]
  \emph{Flächeninhalt}.
\end{defn}
...
\begin{prop}
  ...
  \[ \Int{C}{}{\sqrt{\det(g_u)}}{\mu} = \Int{\phi(C)}{}{\sqrt{\det(\widetilde{g}_u)}}{\mu}. \]
\end{prop}
...
\begin{defn}
  ...
  mit
  \begin{itemize}
    \item $X^s : U \to \R^n$ $\mathcal{C}^2$-Immersion
    \item $X^0 = X$
