...
\begin{defn}
  $X$ heißt \emph{minimal bei konstantem Volumen}, wenn für alle Variationen von $X$ auf derartigen Kompakta $C \subset U$ mit konstantem Volumen gilt: $\delta \A (X^s|_C) = 0$.
\end{defn}
...
$L^2$-Skalarprodukt bzgl. $\d \A = \sqrt{\det(g_u)} \d u$
...
\begin{lem}
  Sei $f : U \to \R$ eine $\mathcal{C}^2$-Abbildung mit $\supp(f) \subset C$. Wenn $\Int{C}{}{f}{\A} = 0$, dann gibt es eine normale Variation $X^s = X + \tau_s$ mit konstantem Volumen, sodass
...
\begin{prop}
  Es sind äquivalent:
  \begin{itemize}
    \item $\delta \A^s = 0$ für jede normale Variation $X^s$ von $X$ auf $C$ mit konstantem Volumen
    \item $\ex{\lambda \in \R} \delta(\A^s) + \lambda \nu^s) = 0$ für alle normalen Variation (auch mit nicht konstantem Volument)
  \end{itemize}
\end{prop}
...
% Bemerkung: Funktionentheorie, differenzierbare Funktionen $\C \to \C$ (holomorphe Funktionen)
...
% Kapitel 3.4. Minimalflächen im $\R^3$
...
\begin{lem}
  ...
  \begin{itemize}
    \item $\triangle X = 2 \lambda^2 H \nu$ % $\lambda$ konformer Faktor, $H$ mittlere Krümmung, $\nu$ Gauß-Abbildung
  \end{itemize}
\end{lem}
...
\begin{lem}
  ...
  \begin{itemize}
    ...
    % d.h. $\partial_{\overline{Z}} (\partial_Z X) = 0$ und $\langle \partial_Z X, \partial_Z X \rangle = 0$
  \end{itemize}
\end{lem}
