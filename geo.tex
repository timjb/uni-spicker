...
\begin{satz}[\emph{Theorema egregium}, Gauß]
...
\begin{satz}[\emph{Codazzi-Mainardi-Gl.}]
...
\begin{satz}[\emph{Hauptsatz der lokalen Flächentheorie (Bonnet)}]\mbox{}\\
...
\begin{prop}
  Der Flächeninhalt ist invariant unter Umparametrisierungen: Sei $X = \widetilde{X} \circ \phi$, $\phi : U \to \widetilde{U}$ ein Diffeo, $C \subset U$ kompakt, dann gilt
...
\begin{defn}
  Sei $X : U \to \R^n$ immergierte $\Cont^2$-Fläche,
  ...
  \[
    \ointerval{-\epsilon}{\epsilon} \times U \xrightarrow{\Cont^2} \R^n, \quad
    (x, u) \mapsto X^s(u) \quad \text{mit}
  \]
  \begin{itemize}
    \miniitem{0.48 \linewidth}{$X^s : U \to \R^n$ $\Cont^2$-Immersion\vphantom{$|_{U \setminus C}$},}
...
\begin{situation}
  ...
  \begin{itemize}
    \miniitem{0.35 \linewidth}{$X^s|_{U \setminus C} = X|_{U \setminus C}$}
    \miniitem{0.62 \linewidth}{$\fa{s \in \ointerval{-\epsilon}{\epsilon}} X^s|_{U_0}$ ist eine Einbettung}
  \end{itemize}
...
% Sei $F : \C \to \C$ eine $\Cont^1$-Abbildung, $F = u + i v$ mit $u, v : \R \to \R$
...
% Für $\Cont^1$-Funktionen $F = (F_1, \ldots, F_n) : U \to \C^n$
...
% Gegeben: $X : U \to \R^3 \subset \C^3$ eine konform parametrisierte minimale $\Cont^2$-Hyperfläche, $U$ zusammenhängend
...
\begin{satz}[\emph{Weierstraß-Darstellung}]
