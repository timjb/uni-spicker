...
\begin{defn}
  ...
  \]
  \begin{itemize}
    \miniitem{0.47 \linewidth}{$X^s : U \to \R^n$ $\mathcal{C}^2$-Immersion\vphantom{$|_{U \setminus C}$}}
    \miniitem{0.17 \linewidth}{$X^0 = X_{\,}$\vphantom{$|_{U \setminus C}$}}
    \miniitem{0.33 \linewidth}{$X^s|_{U \setminus C} = X|_{U \setminus C}$}
  \end{itemize}
\end{defn}
...
\begin{defn}
  ...
  \[ h^\xi : U \to \R^{m \times m}, \quad u \mapsto (\langle \II(\partial_j X(u), \partial_k X(u)), \xi \rangle)_{jk}. \]
\end{defn}
...
\begin{defn}
  Eine Fläche $X : U \to \R^n$ heißt \emph{Minimalfläche} wenn für jedes schöne Kompaktum $C \subset U$ und für jede normale Variation $X^s$ von $X$ auf $C$ gilt: $\delta \A(X^s|_C) = 0$
\end{defn}
