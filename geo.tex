...
% Krümmungstensor in lokalen Variablen
...
\begin{defn}
  Eine lineare Abbildung $F : \R^n \to \R^n$ heißt \emph{konform}, wenn
...
\begin{bem}
  ...
  und es gilt $F$ konform $\iff$ $\tfrac{1}{\mu} A_F \in O(n)$ für ein $\mu \in \R_{> 0}$. Dieses $\mu$ wird Streckungsfaktor genannt.
...
\begin{bem}
  $\lambda : O \to \R_{> 0}$, $\tfrac{1}{\lambda(x)} D_x f \in O(n)$ sei der konforme Faktor.
\end{bem}

% Konforme Abbildungen im $\R^2$

Sei $O \opn \C$ zusammenhängend und $f : O \to \widetilde{O}$ konform. Ohne Einschränkung ist $\det(D_x f) > 0$ für alle $x \in O$ (sonst $\widetilde{f} = f \circ \overline{\cdot}$).
Es folgt: $D_x f = \lambda(x) \begin{psmallmatrix} \cos \alpha_x & \sin \alpha_x \\ -\sin \alpha_x & \cos \alpha_x \end{psmallmatrix} = \lambda(x) \cdot e^{i \alpha_x} \in \C$ und somit ist $f$ holomorph.
