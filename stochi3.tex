...
\begin{bem}
  ...
  \begin{align*}
    ...
    \iff \abs{\overline{X_n} - \mu_0} \leq \tfrac{\sigma}{\sqrt{n}} z_{1-\alpha/2} \\
    &\iff \mu_0 \in \cinterval{\overline{X_n} - \tfrac{\sigma}{\sqrt{n}} z_{1-\alpha/2}}{\overline{X_n} + \tfrac{\sigma}{\sqrt{n}} z_{1-\alpha/2}}.
  \end{align*}
...
\begin{bsp}
  ...
  \[
    \hat{T}(X_1, \ldots, X_n) = \tfrac{\sqrt{n}}{S_n} \left( \overline{X_n} - \mu_0 \right), \quad
    S_n^2 \coloneqq \tfrac{1}{n-1} \sum_{i=1}^n \left( X_i - \overline{X_n} \right)^2
  \]
  Dabei ist $S_n$ die \emph{(korrigierte) Stichprobenvarianz}.
  Man kann zeigen, dass $\hat{T}(X_1, \ldots, X_n) \sim t_{n-1}$.
...
\begin{bem}
  $S_n^2$ und $\overline{X_n}$ sind unabhängig für $n \geq 2$. % und umgekehrt?
\end{bem}
...
\begin{diskussion}
  \begin{itemize}
    ...
    Die Nullhypothese wird nur abgelehnt, falls zu große Stichproben- mittelwerte $\overline{x_n}$ vorliegen. Es ist dann $K_n^T = \ointerval{z_{1-\alpha}}{\infty}$.
  \end{itemize}
\end{diskussion}
