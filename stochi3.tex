...
\]
mit $s_0 = a$, $s_{i-1} < x_i \leq s_i$, $s_n = b$. % Z.B. $s_i = \tfrac{1}{2} (x_{i-1} + x_i)$
...
\]
Schätzung von $f(x,y)$ mittels Produktkern: $K(x, y) = K1(x) \cdot K_2(y)$ mit $K_1$, $K_2$ wie oben. % => $\IInt{\IInt{K(x,y)}{x}}{y} = 1$
\begin{align*}
...
Nächstes Ziel: Bestimmung einer optimalen Bandbreite $h_n$

\begin{satz}
  Sei $f \in \Cont^2(\R^1)$ mit $f'' \in L^2(\R^1)$ und $f, \abs{f'} \leq A < \infty$.
  Für die Kernfunktion $K$ gelte $0 \leq K(x) \leq B$, $K(-x) = K(x)$ und $\IInt{x^2 K(x)}{x} < \infty$.
  Dann gilt für $(h_n)_{n \in \N}$ mit $h_n \downarrow 0$ und $n h_n \to \infty$ für $n \to \infty$:
  \begin{align*}
