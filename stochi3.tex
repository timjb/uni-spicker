...
\begin{modell}[\emph{Lineare Regression}]
  ...
  \[
    \begin{array}{r l}
      Y = (Y_1, \ldots, Y_n)^T & \text{\emph{Beobachtungsvektor},} \\
      X = (x_{ij}) \in \R^{n \times p} & \text{Matrix der \emph{Einstellgrößen}, Versuchsplanmatrix,} \\
      \beta = (\beta_1, \ldots, \beta_p)^T & \text{(unbekannter) \emph{Parametervektor}, Regressionskoeffizienten,} \\
      \epsilon = (\epsilon_1, \ldots, \epsilon_n)^T & \text{(nicht beobachtbarer) \emph{Fehlervektor} heißt.}
    \end{array}
  \]
  Wir nehmen an, dass $\cov(Y_i, Y_j) = \cov(\epsilon_i, \epsilon_j) = \sigma^2 \delta_{ij}$ und dass $n > p$.
