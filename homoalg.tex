...
\begin{defn}
  ...
  Eine Klasse $S \subset \Mor(\Cat)$ von Morphismen heißt \emph{lokalisierend}, wenn
  \begin{enumerate}
    ...
    \item Erweiterungsbedingungen: a) Für alle $(X \xrightarrow{f} Y \xleftarrow{s \in S} Z) \in \Cat$ gibt es $W \!\in\! \Ob(\Cat)$, $(W \xrightarrow{t \in S} X) \in \Cat$ und $(W \xrightarrow{g} Z) \in \Cat$ mit $f \circ t \!=\! s \circ g$. \\[4pt]
    b) Die duale Bedingung zu a)
    \item Für parallele Morphismen $f, g \in \Hom_\Cat(X, Y)$ gilt
    \[ \ex{s \in S} s \circ f = s \circ g \iff \ex{t \in S} f \circ t = g \circ t. \]
  \end{enumerate}
\end{defn}

% III.2.7a)
\begin{bem}
  Aus der Bedingung 2a (oder b) ergibt sich eine vereinfachte Darstellung der Elemente von $\Hom_{\Cat[S^{-1}]}(X, Y)$, wenn $S$ lokalisierend ist: Nämlich kann man in den formalen Ketten alle künstlichen Inversen auf die linke (bzw. rechte) Seite verfrachten. \\
...
\begin{lem}
  ...
  \begin{itemize}
    ...
    \iffalse
    \begin{centertikz}
      \matrix (mat) [matrix of nodes, column sep=1.4cm, row sep=0.3cm]{
        & \node (X') {$X'$}; \\
        \node (X) {$X$}; &&
        \node (Y) {$Y$}; \\
      };
      \draw[->] (X') to node [above] {$s \!\in\! S\quad$} (X);
      \draw[->] (X') to node [above] {$f$} (Y);
    \end{centertikz}
    \fi
...
% TODO: III.2.9 (Bemerkungen zu Dächern!!!)
