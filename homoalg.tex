...
% II.6.5
\begin{prop}
  Sei $\Aat$ eine ab. Kategorie. Dann sind die Funktoren
  \begin{align*}
    \Aat \to \AbGrp, \enspace & Y \mapsto \Hom_\Aat(X, Y) \qquad \text{($X \in \Ob(\Aat)$ fest)}, \\
    \Aat^\op \to \AbGrp, \enspace & X \mapsto \Hom_\Aat(X, Y) \qquad \text{($Y \in \Ob(\Aat)$ fest)}
  \end{align*}
  beide links-exakt.
\end{prop}
...
% II.6.6
\begin{prop}
  Sei $X$ ein topol. Raum, $U \subseteq X$ offen. Der Funktor
  \[ \Sh(X) \to \AbGrp, \enspace \Fais \mapsto \Gamma(U, \Fais) \]
  ist links-exakt.
\end{prop}

\begin{bem}
  Die Prop gilt auch für Garben ab. Gruppen auf Siten.
\end{bem}

% TODO: injektive, projektive, flache Objekte
