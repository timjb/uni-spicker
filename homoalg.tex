...
\begin{bem}
  ...
  \begin{centertikz}
    ...
    \draw[->] (Z) to node [below] {$p_X$} (X);
    \draw[->] (Z) to node [below] {$p_Y$} (Y);
...
\begin{defn}
  Eine \emph{gerichtete Menge} ist eine Menge $X \not= \emptyset$ mit einer Präordnung $\leq$, sodass $\fa{x, y \in X} \ex{z \in X} x \leq z \wedge y \leq z$.
\end{defn}

\begin{defn}
  Ein \emph{projektiver} oder \emph{inverser Limes} ist ein Limes eines Funktors $F$, dessen Quellkategorie die duale Kategorie einer Präordnungskategorie einer gerichteten Menge ist. Notation: $\varprojlim F$
\end{defn}
...
\begin{defn}
  Ein \emph{induktiver} oder \emph{direkter Limes} ist ein Kolimes eines Funktors $F$, dessen Quellkategorie die Präordnungskategorie einer gerichteten Menge ist. Notation: $\varinjlim F$
\end{defn}
...
\begin{lem}
  Sei $\Aat$ eine abelsche Kategorie, $X \xrightarrow{f} Y \xrightarrow{g} Z \in \Aat$ mit $g \circ f = 0$. Sei $(K, k)$ der Kern von $g$ und $(C, c)$ der Kokern von $f$. Deren univ. Eigenschaften induzieren Morphismen $a$, $b$ wie folgt:
  \vspace{-8pt}
  \begin{centertikz}
    \matrix (mat) [matrix of nodes, column sep=1cm, row sep=0.45cm]{
      & \node (C) {$C$}; \\
  ...
  \end{centertikz}
  \vspace{-10pt}
  Es gibt es einen kanonischen Isomorphismus $\coker a \cong \ker b$.
\end{lem}
...
\begin{lem}
  Für ein $A$-Rechtsmodul $X$ sind äquivalent:
  \begin{itemize}
    \item $X$ ist flach.
    \item Alle Relationen von Elementen $x_1, \ldots, x_n \in X$ in einem $A$-Links- modul $Y$ erhält man durch die eben beschriebene Konstruktion.
    \item $X$ ist direkter Limes einer gerichteten Familie von proj. Moduln.
  \end{itemize}
\end{lem}

% II.6.13
\begin{bem}
  Insbesondere sind freie und projektive Moduln (also direkte Summanden von freien Moduln) flach.
\end{bem}
