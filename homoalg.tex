...
\begin{defn}
  \begin{itemize}
    ...
    (\zB{} $\Kom$, $D$, $D^+$, etc.) ist ein Diagramm der Form
...
\begin{lem}
  ...
  \inlineitem{$\Ext^0_\Aat(X, Y) \cong \Hom(X, Y)$}
\end{lem}
...
\begin{defn}
  ...
  \end{centertikzcd}
  der folgende univ. Eigenschaft erfüllt: Für jeden exakten Funktor $F : \Der^+(\Aat) \to \Der^+(\Bat)$ und alle $\epsilon : Q_\Bat \circ K^+(F) \Rightarrow G \circ Q_\Aat$ gibt es ein eindeutiges $\eta : RF \Rightarrow G$, sodass folgendes Diagramm kommutiert:
  \begin{centertikzcd}
