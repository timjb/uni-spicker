...
\begin{defn}
  \begin{itemize}
    ...
    \item Sei $\Aat$ sogar abelsch. Die \emph{$n$-te Homologie} $H_n(\CC{C})$ eines Ketten- komplexes $\CC{C}$ aus $\Aat$ ist der Kokern der Abb. $\delta_n : C_n \to \ker \partial_{n-1}$, die durch die universelle Eigenschaft des Kerns induziert wird.
...
% II.6.3d)
\begin{lem}
  Sei $\Aat$ eine abelsche Kategorie und $0 \to X \xrightarrow{f} Y \xrightarrow{g} Z \to 0 \in \Aat$ eine \keS{}. Dann ist die Sequenz kanonisch isomorph zur Sequenz
  \[ 0 \to \ker g \to Y \to \coker f \to 0. \]
\end{lem}

% II.6.4
\begin{defn}
  Ein additiver Funktor $F : \Aat \to \Bat$ zwischen abelschen Kategorien heißt \emph{exakt}, falls für alle exakten Sequenzen
  \[ 0 \to X \xrightarrow{f} Y \xrightarrow{g} Z \to 0 \enspace \text{in $\Aat$.} \]
  auch die Bildsequenz
  \[ 0 \to FX \xrightarrow{Ff} FY \xrightarrow{Fg} FZ \to 0 \enspace \text{in $\Bat$.} \]
  exakt ist.
\end{defn}

% TODO: links-, rechtsexakt, Beispiele
