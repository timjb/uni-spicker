...
%\usepackage{amssymb} % Doppelpfeil $\rightarrows$
...
\begin{defn}
  Eine Kategorie heißt \emph{endlich}, wenn sie nur endlich viele Objekte und Morphismen besitzt.
\end{defn}
...
\begin{defn}
  ...
  Der Funktor $F$ heißt \emph{darstellbar}, falls ein solches $X \in \Ob(\Cat)$ existiert.
\end{defn}
...
\begin{bem}
  ...
  \[ X \xleftarrow{p_X} Z \xrightarrow{p_Y} Y, \]
...
% II.3.20
\begin{satz}
  Angenommen, eine Kategorie $\Cat$ enthält ein term. Objekt, den Kodifferenzkern von allen parallelen Morphismen $f, g \in \Hom_\Cat(X, Y)$ und das Produkt $X \times Y$ von allen Paaren von Objekten. \\
  Dann existieren alle endlichen Limiten in $\Cat$, \dh{} der Limes von jedem Funktor $F : \Jat \to \Cat$, wobei $\Jat$ endlich ist.
\end{satz}

% II.3.21
\begin{kor}
  In $\SetC$ existieren alle endlichen Limiten.
\end{kor}
