...
\begin{satz}[Harnack-Ungleichung]
  Sei $\Omega \subset \R^n$ offen, $V \subset\subset \Omega$ offen, zusammenhängend. Dann gibt es eine Konstante $c = c(\Omega, V)$, sodass
  \[
...
\begin{satz}[Liouville]
  ...
  \begin{itemize}
    ...
    \item Gilt $\limsup_{\abs{x} \to \infty} \tfrac{\abs{u(x)}}{\abs{x}^{k+1}}$, so ist $u$ ein Polynom, dessen Grad $\leq k$ ist.
  \end{itemize}
\end{satz}

\begin{defn}
  Sei $\Omega \subset \R^n$ offen. Eine Funktion $f : \Omega \to \R$ heißt \emph{analytisch} in $x \in \Omega$, falls $f$ sich lokal durch seine Taylorreihe darstellen lässt, also ein $r \in \ointerval{0}{\dist(x, \partial \Omega)}$ existiert mit
  \[
