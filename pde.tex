...
\begin{bem}[Invarianzen der Lösungseigenschaft]
  Wenn $u \in \mathcal{C}^2_1(\Omega_T)$ kalorisch, dann auch (auf geeigneten Zylindern):
  \begin{alignat*}{3}
...
\begin{bem}
  ...
  \[
    \fa{x_0 \in \R^n} B_r(x_0) = \Set{ x \in \R^n }{ \Phi(x_0 - x) > \Phi(r e_1) }
  \] % "`Superniveaumenge"'
\end{bem}
...
\begin{bem}["`unendliche Ausbreitungsgeschwindigkeit"']\mbox{}\\
  ...
  \[ \min_{\{ 0 \} \times \Omega} u < \max_{\{ 0 \} \times \Omega} u \enspace\implies\enspace \min_{\{ 0 \} \times \Omega} u < u < \max_{\{ 0 \} \times \Omega} u \text{ in $\Omega_T$}. \]
\end{bem}
