...
\begin{bem}[Invarianzen der Lösungseigenschaft]
  ...
  \begin{align*}
    ...
    u_\lambda(t,x) &\coloneqq \lambda^{-1} u(\lambda^2 t, \lambda x) \text{ für } \lambda > 0
  \end{align*}
\end{bem}
...
\begin{bemn}
  \begin{itemize}
    \item $\Psi$ besitzt eine Singularität bei $(0, 0)$.
    \item $\Psi(t, x) \xrightarrow{t \to 0} 0$ bei $x \not= 0$ fest.
    \item Für alle $t \in \R$ gilt
    \[ \Int{\R^n}{}{\Psi(t, x)}{x} = 1. \]
...
\begin{bemn}
  \begin{itemize}
    ...
    \item Explizite Darstellung: $W_r = \{ (t, x) \in \R^{n+1} \mid - \frac{r^2}{4 \pi} < t < 0$ und $\abs{x}^2 < 4t (-u \log r + \tfrac{n}{2} \log (-4 \pi t))\}$
  \end{itemize}
\end{bemn}

\begin{nota}
  $b(t, x) \coloneqq \frac{\abs{x}^2}{4t} + n \log r - \tfrac{n}{2} \log (-4 \pi t)$ für $r > 0$ fest
\end{nota}

\begin{bem}
  Es gilt: $b(t, x) > 0$ in $W_r$ und $b(t, x) = 0$ auf $\partial W_r$.
\end{bem}
...
\begin{lem}
  ...
  \[
    \phi : \ointerval{0}{R} \to \R, \quad
    r \mapsto \frac{1}{4 r^n} \Int{W_r}{}{u(t,x) \tfrac{\abs{x}^2}{t^2}}{(t,x)}
    \qquad \text{gilt dann}
  \]
  \begin{itemize}
    \item $\lim_{r \to 0} \phi(r) = u(0,0)$
    \item $\phi'(r) = \frac{n}{r^{n+1}} \Int{W_r}{}{\left( -u_t(t,x) + \Delta u(t,x) \right) \cdot b(t, x)}{(t,x)}$
  \end{itemize}
\end{lem}

% TODO: Hier fehlt eine Seite, insbesondere Lemmas 3.2 und 3.4.
