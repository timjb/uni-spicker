...
\end{array} \right. \]
mit
\[ - \LL u = - (a_{11}(x, y) u_{xx} + 2 a_{12}(x, y) u_{xy} + a_{22}(x, y) u_{yy}) + b_1(x, y) u_x + b_2(x, y) u_y + c(x, y) u \]
wobei $c(x, y) \leq 0$, $\xi^T A(x, y) \xi \geq \lambda_0 \norm{\xi}^2$, $\lambda_0 > 0$ und $A(x, y) = \begin{pmatrix}
  a_{11}(x,y) & a_{12}(x,y) \\
  a_{21}(x,y) & a_{22}(x,y)
\end{pmatrix}$
...
2. Approximation: $u_{xx}(x, y) \approx \frac{u(x + h, y) - 2 u(x, y) + u(x - h, y)}{h^2}$, $u_{yy}(x, y) \approx \ldots$, $u_x(x, y) \approx \frac{u(x + h, y) - u(x - h, y)}{2h}$, $u_y(x, y) \approx \frac{u(y+h) - u(x, y-h)}{2 h}$,

Mehrere Möglichkeiten für Approx. von $u_{xy}$:

\begin{itemize}
  \item $u_{xy}(x, y) \approx \tfrac{1}{4 h^2} \left( u(x{+}h, y{+}h) - u(x{+}h, y{-}h) - u(x{-}h, y{+}h) + u(x{-}h, y{-}h) \right)$ % 2-mal zentraller DQ, Konsistenzordnung = 2
  % Nachteil: ergibt keine M-Matrix!
  %\item
\end{itemize}
  \begin{align*}
    u_{xy}(x, y) & \approx \tfrac{1}{2 h^2} (- u(x-h, y) + 2 u(x,y) - u(x+h,y) \\
    & + u(x-h,y-h) - u(x, y+h) - u(x,y-h) \\
    & + u(x+h, y+h) ) \text{ falls $a_{12}(x, y) \geq 0$,} \\
    u_{xy}(x, y) & \approx \tfrac{1}{2 h^2} ( u(x-h, y) - 2 u(x, y) + u(x+h, y) \\
    & + u(x, y+h) - u(x-h, y+h) - u(x+h, y-h) \\
    & + u(x,y-h) ) \text{falls $a_{12}(x, y) < 0$}
  \end{align*}

Beziehungsweise als 9-Punkte-Stern
...
\[ \text{(RWP\tss{4})} \enspace \left\{ \begin{array}{rlll}
  u_t(x,t) - \Laplace_x u(x, t) &=& f(x,t) &\text{ in $\Omega = \ointerval{0}{1} \times \ointerval{0}{T}$} \\
  u(x,0) &=& g(x) & x \in \ointerval{0}{1} \\
  u(0, t) &=& g_0(t) & t \in \cinterval{0}{T} \\
  u(1, t) &=& g_1(t) & t \in \cinterval{0}{T}
\end{array} \right. \]

1. Diskretisierung: $x_i = ih$, $h = \nicefrac{1}{n}$, $t_k = k \tau$, $\tau = \nicefrac{T}{m}$, $u(x_i, t_k) \approx u_i^k$
2. Approximation der Ableitungen
...
Einschrittverfahren:
\begin{itemize}
  \item Explizites Euler-Verfahren: $\frac{u_i^{k+1} - u_i^k}{\tau} - \tilde{\Laplace}_h u_i^k = f_i^k$, $u_i^0 = g_h$
  % TODO: Geschweifte Klammer um die beiden Gleichungen (EEV)
  \item Implizites Euler-Verfahren
  $\frac{u_i^{k+1} - u_i^k}{\tau} - \tilde{\Laplace} u_i^{k+1} = f_i^{k+1}$, $u_i^0 = g_h$
  % (IEV)
  \item Crank-Nicolson-Verfahren
  $\frac{u_i^{k+1} - u_i^k}{\tau} - \tfrac{1}{2} \tilde{\Laplace}_h (u_i^k + u_i^{k+1}) = f(x_i, t_k + \tfrac{\tau}{2})$, $u_i^0 = g_h$
  % (CNV)
\end{itemize}
