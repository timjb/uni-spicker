...
% 5.1.16 im Hackenbusch
\begin{satz}
  Sei $c \geq 0$ in $\Omega$ und $\LL$ gleichmäßig elliptisch. \\
...
\begin{verf}
  \begin{enumerate}
    ...
    \]
    berechnen.
    Dazu verwenden wir ein \textit{Einschrittverfahren}, wie das \textit{expl./impl. Gauß-Verfahren} oder das \textit{Crank-Nicolson-Verfahren}:
    \begin{align*}
      \text{\probl{(EEV)}} \enspace
      & \left\{ \begin{array}{R{130pt} c l}
...
\begin{defn}
  Der Raum aller $k$-mal stetig differenzierbaren Funktionen auf~$\Omega$ mit \emph{kompaktem Träger} ist
...
\begin{satz}[Meyers/Serrin]
  $W^{k,p}(\Omega) \cap \Cont^\infty(\Omega)$ liegt dicht in $W^{k,p}(\Omega)$,
  \[
    %H^{k,p} \coloneqq
...
\begin{defn}
  \begin{minipage}[t]{0.8 \linewidth}
    $W^{-1,q} \coloneqq (W_0^{1,p}(\Omega))'$, wobei $\nicefrac{1}{p} + \nicefrac{1}{q} = 1$, \\
    $H^{-1}(\Omega) \coloneqq W^{-1,2} = (H_0^1(\Omega))'$.
  \end{minipage}
\end{defn}

\begin{defn}
  Für $\Omega \subset \R^d$ heißt $\gamma \coloneqq k - \nicefrac{d}{p}$ \emph{Sobolev-Zahl} zu $W^{k,p}(\Omega)$.
\end{defn}

\begin{satz}
  %Für $m < \gamma$ existiert eine stetige Einbettung $W^{k,p}(\Omega) \hookrightarrow \Cont^m(\Omega)$.
  Für $m < \gamma$ ist $W^{k,p}(\Omega) \hookrightarrow \Cont^m(\Omega)$ wohldefiniert und stetig.
\end{satz}
...
\begin{situation}
  ...
  \end{align*}
  Sei $u$ eine Lösung von (RWP\tss{1}) und $\varphi \in \Testfun(\Omega)$.
  Dann gilt
  \begin{align*}
    & \enspace \IntOmega{f(x) \varphi(x)}{x}
    = \enspace \IntOmega{\LL u(x) \varphi(x)}{x} \\
    = & \enspace - \IntOmega{\dive(A(x) \DO u(x)) \varphi(x)}{x} + \IntOmega{\left( b(x) \cdot \DO u(x) + c(x) u(x) \right) \cdot \varphi(x)}{x} \\
    = & \enspace \IntOmega{A(x) \DO u(x) \cdot \DO \varphi(x)}{x} + \IntOmega{\left( b(x) \cdot \DO u(x) + c(x) u(x) \right) \cdot \varphi(x)}{x}
  \end{align*}

  \iffalse
  Nebenrechnung:
  \begin{align*}
    & - \IntOmega{\sum_i \DO_i (\sum_j a_{ij}(x) \DO_j u(x) \varphi(x))}{x} \\
    = & - \IntbOmega{\sum_i (\sum_j a_{ij}(x) \DO_j u(x)) \eta_i(x) \varphi(x)}{s(x)} \\
    & + \IntOmega{\sum_i (\sum_j a_{ij}(x) \DO_j u(x)) \DO_i \varphi(x)}{x} \\
    = & \IntOmega{A(x) \DO u(x) \cdot \DO \varphi(x)}{x}
...
\begin{defn}
  ...
  \begin{align*}
    & \IntOmega{A(x) \DO u(x) \cdot \DO \varphi(x) + b(x) \cdot \DO u(x) \varphi(x) + c(x) u(x) \varphi(x)}{x} \\
    = \enspace & \IntOmega{f(x) \varphi(x)}{x}
    \quad \text{für alle } \varphi \in H_0^1(\Omega).
    \qquad \qquad \text{\probl{(VGL\tss{1})}}
  \end{align*}
\end{defn}

\begin{sprechweise}
  Der Raum, in dem man~$u$ sucht, heißt \emph{Lsgsraum} (oder \textit{Ansatzraum}), der Raum von~$\varphi$ heißt \emph{Testraum}.
\end{sprechweise}
...
\begin{bem}
  ...
  $\ell(\varphi) \coloneqq \IntOmega{f(x) \varphi(x)}{x}$ \enspace
  und
  \[
    B(u, \varphi) \coloneqq \IntOmega{A(x) \DO u(x) \cdot \DO \varphi(x) + b(x) \cdot \DO u(x) \varphi(x) + c(x) u(x) \varphi(x)}{x}
  \]
\end{bem}
...
\begin{defn}
  ...
  \begin{itemize}
    %\item \emph{bilinear}, falls $B(u, \blank)$ und $B(\blank, \varphi)$ linear für alle $u, \varphi \in X$ sind,
    %\item \emph{symmetrisch}, falls $B(u, \varphi) = B(\varphi, u)$ für alle $u, \varphi \in X$,
    \item \emph{positiv}, falls $B(u, u) > 0$ für alle $u \in X \setminus \{ 0 \}$,
    \item \emph{stark positiv} (\textit{koerziv}, \textit{koerzitiv}), falls ein $\lambda > 0$ existiert, sodass
    \[ \fa{u \in X} B(u, u) \geq \lambda \norm{u}_X^2, \]
    \item \emph{beschränkt} (oder \textit{stetig}), falls ein $\mu > 0$ existiert, sodass
    \[ \fa{u, \varphi \in X} \abs{B(u, \varphi)} \leq \mu \norm{u}_X \norm{\varphi}_X. \]
  \end{itemize}
\end{defn}
...
\begin{satz}[\emph{Lax-Milgram}]
  ...
  Dann gibt es für jedes $\ell \in H'$ eine eindeutige Lösung $u \in H$ von  $\fa{\varphi \in H} B(u, \varphi) = \ell(\varphi)$.
...
% 4.11
\begin{lem}
  Falls $\LL$ glm. elliptisch ist und $b(x) \equiv 0$ sowie $c(x) \geq 0$ in~$\Omega$ gelten, so ist~$B$ in (VGL\tss{1})\tprime{} koerziv.
\end{lem}
...
\begin{defn}
  ...
  $B(u, \varphi) = \scp{f}{\varphi}_{H^{-1}(\Omega), H_0^1(\Omega)}$ $\forall \, \varphi \in H_0^1(\Omega)$.
\end{defn}
...
\begin{lem}
  ...
  \[
    ...
    \end{array} \right.
  \]
  für alle $f \in H^{-1}(\Omega)$ eine eind. schwache Lösung $u \in H_0^1(\Omega)$ besitzt.
\end{lem}
...
\begin{prob}
  ...
  Ges. ist $v \in H_0^1(\Omega)$ mit
  \begin{align*}
    & \IntOmega{(A(x) \DO v(x) \cdot \DO \varphi(x) + b(x) \cdot \DO v(x) \varphi(x) + c(x) v(x) \varphi(x))}{x} \\
    = \enspace & \IntOmega{f \cdot \varphi}{x} + \IntOmega{(A(x) \DO \tilde{g} \cdot \DO \varphi + b \cdot \DO \tilde{g} \varphi + c \tilde{g} \varphi)}{x} \quad
    \forall \, \varphi \in H_0^1(\Omega).
  \end{align*}
  ...
  \[
    \underbrace{\IntOmega{A(x) \DO u \cdot \DO \varphi + b \DO u \varphi + c u \varphi}{x}}_{B(u, \varphi) \coloneqq} =
    \underbrace{\IntOmega{f \varphi}{x}}_{\ell(\varphi) \coloneqq} \quad
    \forall \, \varphi \in H_0^1(\Omega).
    \enspace \text{\probl{(VGL\tss{2})}}
  \]
  Für $f \in H^{-1}(\Omega)$ verwendet man
  \[
    B(u, \varphi) = \ell'(\varphi) \coloneqq \scp{f}{\varphi}_{H^{-1}(\Omega), H_0^1(\Omega)} \enspace
    \forall \, \varphi \in H_0^1(\Omega).
...
\begin{prob}
  ...
  \[
    ...
    \end{array} \right.
  \]
  Falls $u \in \Cont^2(\Omega) \cap \Cont(\cOmega)$ eine Lösung ist und $\varphi \in \Cont^\infty(\cOmega)$, so gilt
  \begin{align*}
    & \IntOmega{f \varphi}{x}
    = - \IntOmega{\dive(A(x) \DO u) \varphi + b(x) \DO u \varphi + c(x) u \varphi}{x} \\
    = & - \IntbOmega{A(x) \DO u \cdot \nu \varphi}{s} + \IntOmega{A(x) \DO u \DO \varphi + b(x) \DO u \varphi + c(x) u \varphi}{x}.
  \end{align*}
  Aus der Randbedingung bekommen wir
  \[
    \IntbOmega{A(x) \DO u \cdot \nu \varphi}{s} + \mu \IntbOmega{u \varphi}{s} = \IntbOmega{g \varphi}{s}.
  \]
  Zusammengesetzt erhalten wir die Variationsgleichung
  \[
    \mu \IntbOmega{u \varphi}{s} + \IntOmega{A \DO u \DO \varphi + b \cdot \DO u \varphi + c u \varphi}{x} = \IntOmega{f \varphi}{x} + \IntbOmega{g \varphi}{s}.
  \]
  Wegen Dichtheit von $\Cont^\infty(\cOmega)$ in $H^1(\Omega)$ ist diese Gleichung nicht nur für $\varphi \in \Cont^\infty(\cOmega)$ sondern allgemeiner für $\varphi \in H^1(\Omega)$ erfüllt.
\end{prob}

\begin{defn}
  Sei $\mu \in \R$, $f \in L^2(\Omega)$, $g \in L^2(\bOmega)$.
  Eine Fktn $u \in H^1(\Omega)$ heißt \textit{schwache Lösung} von (RWP\tss{3}), falls für alle $\varphi \in H^1(\Omega)$ gilt:
  \[
    \underbrace{\mu \IntbOmega{u \varphi}{s} + \IntOmega{A \DO u \DO \varphi + b \DO u \varphi + c u \varphi}{x}}_{B(u, \varphi) \coloneqq} =
    \underbrace{\IntOmega{f \varphi}{x} + \IntbOmega{g \varphi}{s}}_{\ell(\varphi) \coloneqq}.
  \]
\end{defn}
...
\begin{bem}
  Ist $\ell(\varphi)$ durch Integration einer Funktion gegeben, so kann man für $\ell_n(\varphi)$ eine Annäherung dieses Integrals etwa mittels der summierten Trapezregel verwenden.
\end{bem}
...
\begin{bem}
  ...
  \begin{enumerate}
    ...
    \]
    Eine Basis von $U_n$ sind die \textit{Hutfunktionen} $w_1, \ldots, w_n$:
    \begin{center}
      \scriptsize
...
\begin{idee}
  ...
  Wir wählen $U_n$ als Raum der Funktionen, die sich als Linearkombination von Basisfktn schreiben lassen, deren Träger nur wenige Teilgebiete umfasst.
\end{idee}
...
% Ciarlet, §2.3
\begin{defn}
  Ein finites Element $(K, P, \Sigma)$ heißt zu einem anderen FE $(\hat{K}, \hat{P}, \hat{\Sigma})$ \emph{affin äquivalent} \textit{vermöge} einer invertierbaren affinen Abbildung $F : \R^d \to \R^d, \enspace \hat{x} \mapsto B \hat{x} + b$, falls gilt:
  \begin{itemize}
    \miniitem{0.3 \linewidth}{$K = F(\hat{K})$}
    \miniitem{0.4 \linewidth}{$P = \Set{p \coloneqq \hat{p} \circ F^{-1}}{\hat{p} \in \hat{P}}$}
    \item $\Sigma = \Set{b_i : p \mapsto b_i(p \circ F)}{\hat{b}_i \in \hat{\Sigma}}$
  \end{itemize}
\end{defn}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item Jede Seite von~$K_i$, \dh{} jedes von~$d$ Eckpunkten von~$K_i$ aufgesp. $(d-1)$-dimensionale Simplex, ist entweder Teil des Gebietrandes $\bOmega$ oder gleichzeitig Seite genau eines anderen Simplex~$K_j$.
  \end{itemize}
\end{defn}
...
\begin{satz}[Abschätzung des lokalen Interpolationsfehlers] \mbox{} \\
  Sei $(\hat{K}, P(\hat{K}), \Sigma(\hat{K}))$ ein finites Element mit $\Sigma(\hat{K}) \subset \Cont^s(\hat{K})'$. \\
  Angenommen, für $r, k \in \N$ mit $0 \leq r \leq k+1$ gilt
  \[
    H^{k+1}(\hat{K}) \hookrightarrow \Cont^s(\hat{K})
    \quad \text{und} \quad
    \mathbb{P}_k(\hat{K}) \subseteq P(\hat{K}) \subset H^r(\hat{K}).
  \]
  Dann existiert ein $C > 0$, sodass für alle zu $(\hat{K}, P(\hat{K}), \Sigma (\hat{K}))$ affin äquivalente FE $(K, P(K), \Sigma(K))$ und für alle $v \in H^{k+1}(K)$ gilt:
  \[
    \abs{v - \Pi_K v}_{H^r(K)} \leq C \cdot \frac{h(K)^{k+1}}{\rho(K)^r} \abs{v}_{H^{k+1}(K)}.
  \]
\end{satz}
...
% 3.1.6 im Ciarlet
\begin{kor}
  Es gibt eine Konstante $\tilde{C} > 0$, sodass für jedes zu $(\hat{K}, P(\hat{K}), \Sigma(\hat{K}))$ affin äquivalente FE $(K, P(K), \Sigma(K))$ mit $h(K) \leq 1$ und jedes $v \in H^{k+1}(K_i)$ gilt:
  \begin{align*}
    \norm{v - \Pi_K v}_{H^r(K_i)} & \leq \tilde{C} \cdot \frac{h(K_i)^{k+1}}{\rho(K_i)^r} \abs{v}_{H^{k+1}(K_i)} \\
...
\begin{voraussetzungen}
  ...
  \begin{enumerate}[leftmargin=2.5em]
    \item[(V\tss{1})] $\cOmega$ ist ein Polyeder und $\mathcal{T} = (T_n(\cOmega))_{n \in \N}$ ist eine Familie von Triangulierungen von~$\Omega$ mit finiten Elementen. \\
    ...
    \item[(V\tss{2})] Alle FE $(K, P(K), \Sigma(K))$ der Familie $\mathcal{T}$ sind affin äquiv. zu einem Referenzelement $(\hat{K}, P(\hat{K}), \Sigma(\hat{K}))$ mit $\Sigma(\hat{K}) \subset \Cont^s(\hat{K})'$.
    \item[(V\tss{3})] $U_n \subset \Cont(\cOmega)$ ist der FE-Funktionenraum zu $T_n(\cOmega)$.
    \item[(V\tss{4})]
    Für $r, k \in \N$ mit $0 \leq r \leq k+1$ gilt
    \[
      H^{k+1}(\hat{K}) \hookrightarrow \Cont^s(\hat{K})
      \quad \text{und} \quad
      \mathbb{P}_k(\hat{K}) \subseteq P(\hat{K}) \subset H^r(\hat{K}).
...
\begin{bem}
  Die konforme Approximation von~$u$ hat gemessen mit der $L^2$-Norm (nach bisherigem Kenntnissstand) im Vergleich zur Interpolation von~$u$ eine um eins schlechtere Konvergenzordnung:
  \begin{alignat*}{3}
...
\begin{lem}[\emph{Aubin-Nitsche}]
  ...
  \[
    B(\varphi, w(r)) = \scp{E(\varphi)}{r}_H \quad \forall \varphi \in U \qquad
    \text{ist.}
  \]
\end{lem}
...
\begin{kor}
  ...
  \[
    B(\varphi, w(r)) = \scp{\varphi}{r}_{L^2(\Omega)} \quad
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item Jede Seite von $K_i$ ist entweder eine Teilmenge von $\bOmega$ oder die Seite von genau einem anderen~$K_j$.
  \end{itemize}
\end{defn}
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item $
      \arraycolsep=2pt
      \begin{array}[t]{r l}
        \Sigma(K) \coloneqq & \Set{p \mapsto p(a_i)}{i = 1, \ldots, d+1} \\
    ...
    \begin{itemize}
      ...
      \item $
        ...
        \begin{array}[t]{r l l l}
          \Sigma(K) \coloneqq & \Set{p \mapsto p(a_i)}{1 \leq i \leq 3} &
          ...
          \cup & \multicolumn{3}{l}{\Set{p \mapsto \DO p(\tfrac{a_i + a_j}{2}) \cdot \nu_{ij}}{1 \leq i < j \leq 3}}
        \end{array}$
       (wobei $\nu_{ij}$ der äußere Normalenvektor an $\nicefrac{(a_i + a_j)}{2}$ ist)
    \end{itemize}
...
Bisher war $B_n = B|_{U_n \times U_n}$.
...
\begin{satz}[1. Lemma von \emph{Strang}]
  Sei $U$ ein Hilbertraum und $(U_n)_{n \in \N}$ eine Familie von Unterräumen.
  Sei $B : U \times U \to \R$ eine beschränkte, koerzitive Bilinearform und $(B_n : U_n \times U_n \to \R)_{n \in \N}$ eine \textit{gleichmäßig koerzitive} Familie beschränkter Bilinearformen, \dh{}
  \[
    \ex{\alpha > 0} \fa{n} B_n(u_n, u_n) \geq \alpha \norm{u_n}^2_U \quad
    \text{für alle $n \in \N$, $u \in U_n$.}
  \]
  Dann existiert eine Konstante $c > 0$, sodass
  \begin{align*}
    & \norm{u - u_n}_U \leq c \left( \inf_{v_n \in U_n} \Phi(v_n) + \norm{\ell - \ell_n}_{U_n'} \right)
    \quad \text{wobei} \\
...
\begin{satz}[Konvergenzsatz für das Modellproblem] \mbox{} \\
  ...
  Sei die Quadraturformel so gewählt, dass $E_{\hat{K}}(p) = 0$ für alle $p \in \Poly_{2k - 2}(\hat{K})$.
  Für die Lösung $u \in H_0^1(\Omega)$ von (VGL) mit $B(u, \varphi) \coloneqq \IInt{\DO u \cdot \DO \varphi}{x}$, $\ell(\varphi) \coloneqq \IInt{f \varphi}{x}$ mit $f \in H^k(\Omega)$ gelte $u \in H^{k+1}(\Omega)$.
  Dann gilt:
  \begin{itemize}
...
\begin{defn}
  ...
  Ein finites Element $(K, P, \Sigma)$ vom Lagrange-Typ (k) heißt \emph{isometrisch äquivalent} zu $(\hat{K}, \hat{P}, \hat{\Sigma})$, falls eine umkehrbare Abbildung
  \[
    F : \hat{K} \to K, \quad
...
\begin{bem}
  Die zu $(K, P, \Sigma)$ isoparametrische Abbildung~$F$ ist für isoparametrische FE vom Lagrange-Typ (k) eindeutig durch die Werte an den Knoten~$\hat{a}_i$ definiert.
\end{bem}
