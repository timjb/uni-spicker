...
\begin{prob}[Allgemeines Variationsproblem] \mbox{} \\
  Seien Abb. $B : H_0^1(\Omega) \times H_0^1(\Omega) \to \R$ und $\ell : H_0^1(\Omega) \to \R$ gegeben. \\
  Gesucht: $u \in H_0^1(\Omega)$, sodass $B(u, \phi) = \ell(\phi)$ für alle $\phi \in H_0^1(\Omega)$.
\end{prob}
...
\TODO{Was ist (VGL\tss{1})'?}

\begin{lem}
  \begin{itemize}
    \item Die Abbildung $B$ in (VGL\tss{1})' ist bilinear und beschränkt.
    \item Die Abbildung $\ell$ in (VGL\tss{1})' linear und stetig.
  \end{itemize}
\end{lem}
...
\begin{prob}
  ...
  \[
    ...
    \underbrace{\IntOmega{f \phi}{x}}_{l(\phi) \coloneqq} \quad
    \forall \, \phi \in H_0^1(\Omega).
    \tag{VGL\tss{2}}
  \]
\end{prob}
...
\begin{prob}
  ...
  \begin{align*}
    \IntOmega{f \phi}{x}
    & = - \IntOmega{\dive(A(x) \DO u) \phi + b \DO u \phi + c u \phi}{x} \\
    & = - \IntbOmega{A(x) \DO u \cdot \nu \phi}{s} + \IntOmega{A(x) \DO u \DO \phi}{x} + \IntOmega{b(x) \DO u \phi}{x} + \IntOmega{c u \phi}{x}.
  \end{align*}
  ...
  \[
    \mu \IntbOmega{u \phi}{s} + \IntOmega{A(x) \DO u \DO \phi + b(x) \cdot \DO u \phi + c(x) u \phi}{x} = \IntOmega{f \phi}{x} + \IntbOmega{g \phi}{s}.
  \]
...
\begin{defn}
  ...
  \[
    \mu \IntbOmega{u \phi}{s} + \IntOmega{A \DO u \O \phi + b \DO u \phi + c u \phi}{x} =
    \IntOmega{f \phi}{x} + \IntbOmega{g \phi}{s} \quad
...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item Jeder Rand von $K_i$, \dh{} jedes $(d+1)$-dimensionale Simplex mit $d$ Eckpunkten von $K_i$ ist entweder Teil des Gebietrandes $\bOmega$ oder der vollständige Rand eines anderen Simplex $K_j$.
  \end{itemize}
\end{defn}

\begin{defn}
  Ein Raum der linearen finiten Elemente zu einer Triangulierung $T(\cOmega)$ ist definiert durch
  \[
    %U_n = \Set{v \in \Cont(\cOmega)}{v|_{K_i} \text{ ist ein Polynom der Ordnung 1 für } i = 1, \ldots, N}
...
\begin{satz}[\emph{Allgemeiner Finite-Elemente-Raum}]
  Sei $T(\cOmega) = \Set{(K_i, P(K_i), \Sigma(K_i))}{i = 1, \ldots, N}$ eine zulässige Triangulierung und $\mathcal{K}_k = \bigcup_{i=1}^N \mathcal{K}_k(K_i) = \Set{\tilde{a}_i}{i = 1, \ldots, N}$ das Lagrange-Gitter $k$-ter Ordnung, wobei
  \[
  ...
  Dann ist durch Vorgabe von $v|_{\mathcal{K}_k}$ eindeutig ein $v \in U_n \subset H^1(\Omega)$ mit $U_n = \Set{v \in \Cont(\cOmega)}{v|_{K_i} \in \Poly_k(K_i), i = 1, \ldots, N}$ bestimmt.
...
\[
  ...
  \quad \text{mit} \quad
  B_n(u_n, \phi) = \IntOmega{\D u_n \cdot D \phi}{x}, \enspace
  l_n(\phi) = \IntOmega{f(x) \phi(x)}{x}
\]
...
\begin{defn}
  ...
  Dann heißt $\Pi \omega$ \emph{$U_n$-Interpolierend} von $\omega \in \Cont^s(\cOmega)$, falls $\Pi \omega = \sum_{i=1}^n b_i(\omega) p_i \in U_n$.
\end{defn}

\begin{lem}
  Für alle $K_i \in T(\cOmega)$ und alle $\omega \in \Cont^3(\cOmega)$ gilt $(\Pi \omega) |_K = \Pi_{K_i} \omega|_{K_i}$ und somit $\norm{\omega - \Pi \omega}_{H'(\Omega)} = \sum_{K_i \in T(\cOmega)} \norm{\omega - \Pi_{K_i} \omega}_{H'(K_i)}$.
\end{lem}
