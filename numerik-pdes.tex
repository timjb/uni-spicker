...
wobei $c(x, y) \leq 0$, $\xi^T A(x, y) \xi \geq \lambda \norm{\xi}^2$ und $A(x, y) = \begin{pmatrix}
  a_{11}(x,y) & a_{12}(x,y) \\
  a_{21}(x,y) & a_{22}(x,y)
\end{pmatrix}$
...
\begin{itemize}
  \item $u_{xy}(x, y) \approx \frac{u(x + h, y+h) - u(x + h, y - h) - u(x-h, y+h) + u(x-h, y-h)}{4 h^2}$ % 2-mal zentraller DQ, Konsistenzordnung = 2
  % Nachteil: ergibt keine M-Matrix!
  \item $u_{xy}(x, y) \approx \tfrac{1}{2 h^2} \left(- u(x-h, y) + 2 u(x,y) - u(x+h,y) + u(x-h,y-h) - u(x, y+h) - u(x,y-h)+u(x+h, y+h) \right)$ falls $a_{12}(x, y) \geq 0$,
  $u_{xy}(x, y) \approx \tfrac{1}{2 h^2} \left( u(x-h, y) - 2 u(x, y) + u(x+h, y) + u(x, y+h) - u(x-h, y+h) - u(x+h, y-h) + u(x,y-h) \right)$ falls $a_{12}(x, y) < 0$
\end{itemize}
...
\[ - \LL u(x, y) \approx \tfrac{1}{h^2} \left( (-a_{11} + \abs{a_{12}} + \nicefrac{h}{2} b_1) u(x-h, y) + (-a_{11} + \abs{a_{12}} - \nicefrac{h}{2} b_1) u(x + h, y) + (2 a_{11} + 2 a_{22} - 2 \abs{a_{12}} + h^2 c) u(x, y) + (-a_{22} + \abs{a_{12}} + \nicefrac{h}{2} b_2) u(x, y-h) + (-a_{22} + \abs{a_{12}} - \nicefrac{h}{2} b_2) u(x, y+h) + (- \max(a_{12}, 0) u(x-h, y-h) + (- \max(a_{12}, 0)) u(x+h, y+h) + \min(a_{12}, 0) u(x-h, y+h) + \min(a_{12}, 0) u(x + h, x - h)) \right) \]
