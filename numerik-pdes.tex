...
\begin{satz}[\emph{Höldersche Ungleichung}]
  ...
  Dann ist $uv \in L^r(\Omega)$ und es gilt
  \[ \norm{uv}_r \leq \norm{u}_p \cdot \norm{v}_q. \]
\end{satz}
...
$\Omega \subset \R^d$ beschränkt

\[
  \text{(RWP\tss{1})} \left\{ \begin{array}{rlll}
    \LL u &=& f &\text{ in $\Omega$} \\
    u &=& 0 &\text{ auf $\bOmega$}
  \end{array} \right.
\]
...
Sei $\phi \in \Testfun(\Omega)$
\[
  \Int{\Omega}{}{\LL u \phi}{x} = - \Int{\Omega}{}{\dive(A(x) \DO u(x)) \phi(x)}{x} + \Int{\Omega}{}{b(x) \cdot \DO(u(x)) \phi(x)}{x} + \Int{\Omega}{}{c(x) u(x) \phi(x)}{x}
\]

\[
  \Int{\Omega}{}{f(x) \phi(x)}{x} = \Int{\Omega}{}{A(x) \DO u(x) \cdot \DO \phi(x)}{x} + \Int{\Omega}{}{b(x) \cdot \DO u(x) \phi(x)}{x} + \Int{\Omega}{}{c(x) u(x) \phi(x)}{x}
\]

Nebenrechnung:
\begin{align*}
  - \Int{\Omega}{}{\sum_i \DO_i (\sum_j a_{ij}(x) \DO_j u(x) \phi(x))}{x}
  & = - \Int{\partial \Omega}{}{\sum_i (\sum_j a_{ij}(x) \DO_j u(x)) \eta_i(x) \phi(x)}{s(x)} + \Int{\Omega}{}{\sum_i (\sum_j a_{ij}(x) \DO_j u(x)) \DO_i \phi(x)}{x} \\
