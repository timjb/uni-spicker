...
\[
  \left\{ \begin{array}{rl}
    \partial_{tt} u - c^2 \partial_{xx} u = f(x,t) \\
    u(0, t) = g_0(t), u(1, t) = g_1(t) & \text{für $t \in \cinterval{0}{T}$} \\
    u(x, 0) = q_0(x), u_t(x, 0) = q_1(x) & \text{für $ \in \ointerval{0}{1}$}
    %\LL u &=& f &\text{ in $\Omega$} & \text{(PDE)} \\
    %\RR u &=& g &\text{ auf $\bOmega$} & \text{(Randbedingung)}
  \end{array} \right.
\]
...
\begin{enumerate}
  ...
  \[
    ...
    \end{array} \right.
  \]
  % (ausgelassen: wie man jetzt ein LGS bekommt)
\end{enumerate}

\begin{bem}
  Das Differenzenverfahren \ldots
  \begin{itemize}
    \item \ldots{} ist einfach in der Herleitung und Implementierung.
    \item \ldots{} besitzt eine gute Konvergenz (\zB{} Ordnung 2) bei genügend glatter Lösung.
    \item \ldots{} ermöglicht Adaptivität bzw. unregelmäßige Gitter nur sehr schwer.
  \end{itemize}
\end{bem}

% Vorlesung vom 17.11.2015

\section{Schwache Lösungstheorie für elliptische DGLn}
...
\begin{defn}
  \begin{itemize}
    \item Für $1 \leq p < \infty$ heißt
    \begin{align*}
      & L^p(\Omega) \coloneqq \Set{v : \Omega \to \R}{\norm{v}_p < \infty}, \quad \text{mit}
      & \norm{v}_p \coloneqq \left( \Int{\Omega}{}{\abs{v(x)}^p}{x} \right)^{\nicefrac{1}{p}}
    \end{align*}
    heißt \emph{$L^p$-Raum}.
    \item $L^\infty(\Omega) \coloneqq \Set{v : \Omega \to \R}{\norm{v}_\infty < \infty}$,
...
\begin{bemn}
  \begin{itemize}
    \item $(L^p(\Omega), \norm{\blank}_p)$ ist eine Banachrau, für $p = 2$ sogar ein Hilbertraum mit Skalarprodukt $\scp{u}{v}_{L^2(\Omega)} \coloneqq \Int{\Omega}{}{u(x) v(x)}{x}$.
  \end{itemize}
\end{bemn}
...
\begin{defn}
  Die Menge aller $k$-mal stetig diff'baren Funktionen auf $\Omega$ mit \emph{kompaktem Träger} ist
  \[ \Cont_0^k(\Omega) \coloneqq \Set{\varphi \in \Cont^k(\Omega)}{\supp(\varphi) \coloneqq \clos{\Set{x \in \Omega}{\varphi(x) \neq 0}}} \]
\end{defn}

\begin{defn}
  $\mathcal{D}(\Omega) \coloneqq \Cont^\infty_0 (\Omega)$ heißt Raum der \emph{Testfunktionen} in $\Omega$
\end{defn}
...
\begin{defn}
  $L_\loc^1(\Omega) \coloneqq \Set{v : \Omega \to \R}{v|_K \in L^1(K) \text{ für jede kompakte Teilmenge $K \subset \Omega$}}$
\end{defn}

\begin{defn}
  Sei $u \in L^1_\loc(\Omega)$ und $\alpha \in \N^n$.
  Eine Funktion $v \in L^1_\loc(\Omega)$ heißt \emph{schwache (partielle) Ableitung} von $u$ der Ordung $\alpha$ (oder die Ableitung im distributionellen Sinn), wenn
  \[
    \Int{\Omega}{}{\DO^\alpha \varphi(x)}{x} = (-1)^{\abs{\alpha}} \Int{\Omega}{}{v(x)}{\varphi(x)}{x}
    \quad \text{für alle $\varphi \in \mathcal{D}(\Omega)$}.
  \]
\end{defn}

\begin{bem}
  Ist eine Funktion im klassischen Sinne diff'bar, so stimmen die schwachen und klassischen Ableitungen überein.
\end{bem}

\begin{lem}[Fundamentallemma der Variationsrechung]
  Sei $u \in L^1_\loc$ und $\Int{\Omega}{}{u(x) \varphi(x)}{x} = 0$ für alle $\varphi \in \mathcal{D}(\Omega)$.
...
\begin{lem}
  \begin{itemize}
    \item $\DO^\alpha (u + \lambda v) = \DO^\alpha u + \lambda \DO^\alpha v$
    \item $\DO^{\alpha + \beta} u = \DO^\alpha (\DO^\beta u)$
  \end{itemize}
\end{lem}
...
\begin{defn}
  ...
  Dann heißt der Raum
  \begin{align*}
    W^{k,p}(\Omega) & = \Set{u \in L^p(\Omega)}{\begin{array}{l}
      \fa{\alpha \in \N^n \text{ mit } \abs{\alpha} \leq k} \\
      \exists \, \text{schwache Abl. $\DO^\alpha u \in L^p(\Omega)$}
    \end{array}} \\
