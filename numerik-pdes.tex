...
\begin{verf}[Dividierte Differenzen von Newton]\mbox{}\\
  Für $u \in \Cont^3(\cinterval{x_l}{x_r})$ ist

  \begin{align*}
    u''(x) = & \frac{2}{x_r - x_l} \left( \frac{u(x_r) - u(x)}{x_r - x} - \frac{u(x) - u(x_l)}{x - x_l} \right) + \O(x_r - x_l) \\
  ...
  \begin{align*}
    \DO_h u(x, y) = \nicefrac{1}{h^2} & \left( \frac{2}{s_l (s_r + s_l)} u(x - s_l h, y) + \frac{2}{s_r (s_r + s_l)} u(x+s_r h, y) \right. \\
    & \left. + \frac{2}{s_u (s_o + s_u)} u(x, y - s_u h) + \frac{2}{s_o (s_o + s_u)} u(x, y + s_o h) \right. \\
    & \left. - (\frac{2}{s_l s_r} + \frac{2}{s_o s_u}) u(x, y) \right)
  \end{align*}

  wobei
  \begin{align*}
    & x_r - x_l = (s_r + s_l) h, \quad
    x_r - x = s_r h, \quad
    x - x_l = s_l h \\
    & y_o - y_u = (s_o - s_u) h, \quad
    y_o - y = s_o h, \quad
    y - y_u = s_u h
  \end{align*}
  ...
  \[
    \text{(RWP\tss{2})'\tss{h}}\enspace \left\{ \begin{array}{rl}
      - \DO_h u_h = f_h &\text{ in $\Omega_h$} \\
      u_h = g &\text{ auf $\bOmega_h$}
    \end{array} \right.
  \]
  \[
    \text{(LGS\tss{2})'}\enspace \left\{ \begin{array}{rl}
      - \tilde{\DO}_h \tilde{u}_h & = \tilde{f}_h \\
      \tilde{f}_h & = f_h + g_h
    \end{array} \right.
  \]
  \[ g_h(x, y) = \nicefrac{1}{h^2} \sum_{(x_N, y_N) \in N(x, y) \cap \partial \Omega_h} S_{x_N,y_N} g(x_N, y_N) \]
  mit
  \[
    ...
    \end{cases}
  \]
  % N(x, y) := Menge der Nachbarn von (x, y)
  \[
    - \tilde{\DO}_h = (d_{ij}) \quad \text{mit} \quad
    d_{ii} = \nicefrac{1}{h^2} \left( \tfrac{2}{s_{il} s_{ir}} + \tfrac{2}{s_{iu} s_{io}} \right),
  \]
  \[
...
\begin{bem}
  Das DV (RWP\tss{2})'\tss{h} hat in den randnahen Punkten nur die Konsistenzordnung 1.
  Sei $u \in \Cont^4(\cOmega)$.
  Dann ist
  \[ u(x + s_r h, y) = u(x, y) + s_r h u_x(x, y) + \tfrac{s_r^2 h^2}{2} u_{xx}(x,y) + \tfrac{s_r^3 h^3}{6} u_{xxx}(x, y) + \tfrac{s_r^4 h^4}{24} u_{xxxx}(\xi_1, y) \]
  für ein $\xi_1 \in \ointerval{x}{x+s_r h}$.
  \[ u(x - s_l h, y) = u(x, y) - s_l h u_x(x, y) + \tfrac{s_l^2 h^2}{2} u_{xx}(x, y) - \tfrac{s_l^3 h^3}{6} u_{xxx}(x, y) + \tfrac{s_r^4 h^4}{24} u_{xxxx}(\xi_2, y), \]
  $\xi_2 \in \ointerval{x - s_l h}{x}$.
  Es folgt
  \[ \nicefrac{1}{h^2} \left( \tfrac{2}{s_l (s_l + s_r)} u(x - x_l h, y) + \tfrac{2}{s_r (s_r + s_l)} u(x+s_r h, y) - \tfrac{2}{s_r s_l} u(x, y) \right) = u_{xx}(x, y) + \tfrac{1}{3} (s_r - s_l) h u_{xxx}(x, y) + \tfrac{1}{12} h^2 \left( \tfrac{s_r^3 u_{xxxx}(\xi_1, y)}{s_r + s_l} + \tfrac{s_l^3 u_{xxxx}(\xi_2, y)}{s_r + s_l} \right) \]
  % 0)
  \[ \nicefrac{1}{h^2} \left( \tfrac{2}{s_l (s_r + s_l)} + \tfrac{2}{s_r (s_l + s_r)} + \tfrac{2}{s_r sl} \right) u(x, y) = 0 \]
  % 1)
