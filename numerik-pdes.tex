...
Wir untersuchen nun das inhomogene RWP
...
\begin{align*}
  ...
  = & \Int{\Omega}{}{f(x) \phi(x)}{x} + \Int{\Omega}{}{(A(x) \DO \tilde{g}(x) \DO \phi(x) + b \DO \tilde{g} \phi + c \tilde{g} \phi)}{x} \quad
  \forall \, \phi \in H_0^1(\Omega).
\end{align*}

Voraussetzungen: $a_{ij}, b_i, c \in L^\infty(\Omega)$, $f \in L^2(\Omega)$, $\tilde{g} \in \Cont(\cOmega) \cap H^1(\Omega)$.

Gesucht ist also ein $u \in U \coloneqq \Set{w \in H^1(\Omega)}{\tau(w) = g}$. % Dabei ist $\tau : H^1(\Omega) \to L^2(\bOmega)$ der Spuroperator

\[
  \underbrace{\Int{\Omega}{}{A(x) \DO u \cdot \DO \phi + b(x) \DO u \phi + c(x) u \phi}{x}}_{B(u, \phi) \coloneqq} =
...
\[
  ...
  \left\{ \begin{array}{r c l l}
    \LL u & = & f & \text{in } \Omega, \\
    A(x) \DO u \cdot |nu + \mu u & = & g & \text{auf } \bOmega
  \end{array} \right.
\]

$u \in \Cont^2(\Omega) \cap \Cont(\cOmega)$, $\phi \in \Cont^\infty(\cOmega)$
...
\[
  \mu \Int{\partial \Omega}{}{u \phi}{s} + \Int{\Omega}{}{A(x) \DO u \DO \phi + b(x) \cdot \DO u \phi + c(x) u \phi}{x} = \Int{\Omega}{}{f \phi}{x} + \Int{\partial \Omega}{}{g \phi}{s} \quad
  \forall \, \phi \in C^\infty(\cOmega).
\]

Da $\Cont^\infty(\bOmega)$ dicht in $H^1(\Omega)$ liegt, ...

\begin{defn}
  Sei $\mu \in \R$, $f \in L^2(\Omega)$, $g \in L^2(\partial \Omega)$.
  Eine Funktion $u \in H^1(\Omega)$ heißt schwache Lösung von (RWP\tss{3}), falls
  \[
