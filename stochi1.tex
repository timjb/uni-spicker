...
\begin{satz}
  Falls $f : (\Omega, \Alg) \to (\ER, \overline\Bor)$ messbar, dann auch $\abs{f}$, $f^+$ u. $f^-$.
\end{satz}
...
\begin{satz}
  Es sind messbar: \enspace
  \begin{minipage}[t]{0.6 \linewidth}
    \inlineitem{Monotone Fktn,} \enspace
    \inlineitem{Càdlàg-Fktn,} \\
    \inlineitem{Fktn von beschränkter Variation}
  \end{minipage}
\end{satz}
...
\begin{defn}
  ...
  \[ P_X : \LebAlg(\R^1) \to \I, \quad B \mapsto \P(\{ X \in B \}) = \P(X^{-1}(B)) \]
...
\begin{beweis}
  \begin{enumerate}
    ...
    \item Möglichkeit: Wähle $\Omega \coloneqq \I$, $\Alg \coloneqq \Leb(\I)$, $\P \coloneqq \lambda_1$. Setze
    \[
...
\begin{defn}
  Sei $(\Omega, \Alg, \mu)$ ein Maßraum und $f : (\Omega, \Alg) \to (\ER, \mathcal{L}(\ER))$. Für $p \in \cointerval{1}{\infty}$ heißt $f$ \emph{$p$-integrierbar}, falls $\abs{f}^p$ $\mu$-integrierbar ist.
  \begin{align*}
...
\begin{satz}
  Der $L^p(\mu)$ ist ein vollständiger normierter Raum, \dh{} jede Cauchy-Folge bzgl. der Norm $\norm{\blank}_p$ ist auch konvergent.
\end{satz}
...
\begin{satz}
  Der Erwartungswert ist linear, \dh{} für ZGn $X$ und $Y$ und $\lambda \in \R$ gilt $\E (\lambda X + Y) = \lambda \E X + \E Y$.
\end{satz}
...
\begin{defn}
  ...
  \[ F = F_{(X_1, ..., X_d)} : \R^d \to \I, \quad (x_1, ..., x_d) \mapsto \P(X_1 {\leq} x_1, ..., X_k {\leq} x_k) \]
...
\begin{defn}
  ...
  \[ \fa{x, y \in J} \fa{t \in \I} g(t a + (1{-}t)b) \leq t g(a) + (1{-}t) g(b) \]
\end{defn}

\begin{satz}
  Sei $g : \R^1 \to \R^1$ konvex auf einem Intervall $J$ und $X$ eine ZG mit $\P(X \in J) = 1$ und $\P \abs{X} < \infty$. Dann gilt für $x, y \in I$, $t \in \I$:
  \[
    g(\E X) \leq \E g(X)
...
\begin{frage}[Momentenproblem (MP)]
  Unter welchen Bedingungen ist eine Folge $(c_i)_{i \in \N}$ eine Momentenfolge einer ZG $X$ \dh{} $c_i = \E X^i$?
\end{frage}
...
\begin{satz}
  \begin{itemize}
    ...
    \item Falls $X, Y$ unabhängig, dann gilt $\cov(X, Y) = \cor(X, Y) = 0$.
    \item $\cor(X, Y) = +1 \iff \ex{a > 0, \, b \in \R} \P(Y = aX + b) = 1$
    \item $\cor(X, Y) = -1 \iff \ex{a < 0, \, b \in \R} \P(Y = aX + b) = 1$
  \end{itemize}
\end{satz}
...
\iffalse
  ...
  Die Funktion
  \[ f_a(y) \coloneqq f_Y(y) \cdot (1 + a \cdot \sin(2 \pi \log y)), y \in \R_{>0}^1, a \in \I \]

  Für $a \in \I$ ist dann $\Int{-\infty}{\infty}{f_a(y)}{y} = 1$
...
\iffalse
  \begin{bsp}[Box-Muller-Transformation]
    ...
    \end{align*}
    mit $X_1$ und $X_2$ gleichverteilt auf $\I$.

    \[ f_{(X_1)}(x_1, x_2) = \ind_{\ointerval{0}{1}} \]
  ...
  \begin{bsp}
    ...
    Ergebnis: $f_{(Y_1, Y_2)}(y_1, y_2) = \frac{1}{2 \pi} \cdot \underbrace{\frac{y_1}{\sigma^2} \exp(- \frac{y_1^2}{\sigma^2})}_{\text{Dichte der Rayleigh-Verteilung}}$ für $(y_1, y_2) \in \ointerval{0}{\infty} \times \ocinterval{0}{2\pi}$
...
\begin{satz}
  ...
  \begin{align*}
    ...
    f_{Z_3}(z) &= \Int{\mathclap{-\infty}}{\infty}{f_{(X, Y)}(z y, y) \cdot \abs{y}}{y} \overset{\text{Unabh.}}= \Int{\mathclap{-\infty}}{\infty}{f_X(zy) \cdot f_Y(y) \cdot \abs{y}}{y}
  \end{align*}
\end{satz}
...
\iffalse
  % Gewisse Verteilungen sind "`faltungsstabil"', \dh{} bei der Faltung bleibt der Verteilungstyp erhalten
  \begin{bsp}
...
\iffalse
  $\Omega_1 = \Omega_2 = \I$, $\mu_1 = \mu_2 = \lambda_1$
  \begin{itemize}
  ...
  Sei $T = \I$, $(X_t)_{t \in T}$ eine Familie unabhängiger, $N(0, 1)$-vertielter ZGn über eiem W-Raum $(\Omega, \Alg, \P)$.
