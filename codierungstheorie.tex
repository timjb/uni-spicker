...
\begin{satz}
  ...
  \[
    ...
    \end{pmatrix}
  \]
  Dann ist $\Omega$ ein $[12, 6, 6]$-Code über $\F_3$, selbstdual.
\end{satz}
...
\begin{defn}
  Eine \emph{Inzidenzstruktur} ist ein Tupel $\mathcal{D} = (V, \mathcal{B}, I)$ bestehend aus
  \begin{itemize}
    \item einer (nichtleere) Menge von \emph{Punkten} (oder \textit{Knoten}) $V$,
    \item einer (nichtleere) Menge von \emph{Blöcken} (oder \textit{Geraden}) $\mathcal{B}$ und
    \item einer \emph{Inzidenzrelation} $I \subseteq V \times \mathcal{B}$
  \end{itemize}
\end{defn}
...
\begin{bem}
  Wir können die Inzidenzrelation durch eine Matrix $M \in \C^{\abs{V} \times \abs{\mathcal{B}}}$ darstellen, welche Einträge in $\{ 0, 1 \}$ besitzt.
\end{bem}

\begin{nota}
  $\sigma(p) \coloneqq \Set{B \in \mathcal{B}}{p I B}$ \\
  $\Sigma(B) \coloneqq \Set{p \in V}{p I B}$
\end{nota}

\begin{defn}
  $\mathcal{D}$ heißt \emph{einfach}, falls $\Sigma$ injektiv ist.
\end{defn}

\begin{nota}
  Falls $\mathcal{D}$ einfach: $p \in B \coloniff p I B$
\end{nota}

\begin{nota}
  $v \coloneqq \abs{V}$, $b \coloneqq \abs{\mathcal{B}}$
\end{nota}

\begin{defn}
  Eine endliche Inzidenzstruktur $\mathcal{D} = (V, \mathcal{B}, I)$ heißt \emph{linearer Raum}, falls gilt:
  \begin{itemize}
    \item $\abs{\Sigma(B)} \geq 2$ für alle $B \in \mathcal{B}$
...
\begin{satz}
  $\mathcal{D} = (V, \mathcal{B}, {\in})$ sei ein linearer Raum mit $b \leq 2$.
  Dann gilt $b \geq v$.
\end{satz}
...
\begin{bsp}
  ...
  \begin{align*}
    ...
    \mathcal{B} = \{ L_1, \ldots, L_{v-1}, B \} \\
    & \Sigma(L_i) = \{ x_i, x_v \}, \quad
    \Sigma(B) = \{ x_1, \ldots, x_{v-1} \}
  \end{align*}
  ist ein linearer Raum mit $b = v$.
  Dieser heißt \emph{entartet}.
\end{bsp}
