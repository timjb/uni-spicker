...
\begin{satz}
  Für $m \geq 1$ gilt $A_2^\lin(2^m, 2^{m-1}) = A_2(2^m, 2^{m-1}) = 2^{m+1}$, \dh{} es existiert ein $[2^m, m+1, 2^m]$-Code.
\end{satz}
...
\begin{bem}
  ...
  \]
  ist ein surj. Algebra-Homomor. mit Kern $\langle x_1^2 + x_1, \ldots, x_m^2 + x_m \rangle$.
  Somit gilt
