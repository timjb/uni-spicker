...
\begin{satz}
  Sei~$C$ ein linearer $[n, k, d]$-Code über $\F_q$, $H$~eine Kontrollmatrix zu~$C$.
  Dann gilt:
  \begin{align*}
...
\begin{defn}
  ...
  Die \emph{Gewichtsverteilung von $C$} ist $A = A_C \in \N^{\{ 0, 1, \ldots, n \}}$ mit $A(i) \coloneqq $ Anzahl der Codeworte in $C$ mit Gewicht~$i$ ($0 \leq i \leq n$).
\end{defn}
...
% Nun $t=1$, also $d=3$. Ist nun $C$ perfekt, so gilt $\abs{C} = \frac{q^n}{1 + n(q-1)}$, es ist also $1+n(q-1)$ ein Teiler von $q^n$.

% Für $q \geq 2$ und $n = q+1$ ist $1 + n(q-1) = q^2$ ein Teiler von $q^n$

% Das ist eine notwendige, aber nicht hinreichende Bedingung. Zum Bsp gibt es keinen perfekten Code bei $q=6$.

\begin{lem}
  Seien $p, u, v \in \N$, $p \geq 2$. Dann gilt $u \divides v \iff p^n - 1 \divides p^v - 1$.
\end{lem}
...
Sei $q \geq 2$ eine Primzahlpotenz, $C$ ein $q$-närer perfekter $(n, M, 3)$-Code.
Dann $q^k = \abs{C} = M = \frac{q^n}{1 + n(q-1)}$ $\iff$ $n = \frac{q^{n-k} - 1}{q - 1}$

Wie viele Lösungspaare $(n, k)$ gibt es bei festem $q$?

Ein Parameter $m \in \N$, $m \geq 2$: $k(m) \coloneqq n - m$, $n(m) \coloneqq \tfrac{q^m - 1}{q - 1}$.
