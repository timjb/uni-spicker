...
\DeclareMathOperator{\Graph}{Graph} % Hesse-Matrix
\DeclareMathOperator{\Hom}{Hom} % Hesse-Matrix
...
\begin{defn}
  Sei $f : \cinterval{a}{b} \to \R$ beschränkt. Dann heißen

  \begin{align*}
    \Upintabdx{f(x)} &\coloneqq \inf\left\{ \Intabdx{\phi(x)} : \phi \in \mathcal{T}{\cinterval{a}{b}}, \phi \ge f \right\} \\
    \Lowintabdx{f(x)} &\coloneqq \sup\left\{ \Intabdx{\phi(x)} : \phi \in \mathcal{T}{\cinterval{a}{b}}, \phi \le f \right\}
  \end{align*}
...
\begin{bem}
  ...
  \begin{satz}
  Für $f, f_{1}, f_{2} : \cinterval{a}{b} \to \R$ beschränkt und $\lambda \ge 0$ gilt
...
\begin{defn}
  Eine Abb. $f : \cinterval{a}{b} \to \R$ heißt \emph{Riemann-integrierbar}, wenn
...
\begin{satz}[$\triangle$-Ungleichung für das Riemann-Integral]
  Für $f, g \in \mathcal{R}_{\cinterval{a}{b}}$:
  \[ \Intabdx{\abs{f(x) + g(x)}} \le \Intabdx{\abs{f(x)}} + \Intabdx{\abs{g(x)}}. \]
\end{satz}

\begin{satz}[Vertauschung von Integration und Limes bei glm. Konv.]
  ...
  \begin{itemize}
    ...
    \begin{align*}
      \norm{f}_{\text{op}} &\coloneqq \sup \Set{ \frac{ \norm{f(x)}_W }{ \norm{x}_V } }{ x \in V \backslash \{ 0 \} }\\
        &= \sup \Set{ \norm{f(x)}_W }{ x \in V, \norm{x}_V = 1 }
    \end{align*}
    %\[ \norm{f}_{\text{op}} \coloneqq \sup\left\{ \frac{ \norm{f(x)}_W }{ \norm{x}_V } : x \in V \backslash \{ 0 \} \right\} = \sup \left\{ \norm{f(x)}_W : x \in V, \norm{x}_V = 1 \right\} \]
    auf $\Hom(V, W)$ heißt \emph{Operatornorm}.
  \end{itemize}
\end{bem}

%\begin{nota}
%Im Folgenden bezeichne $(V, \norm{\blank})$ einen normierten VR.
%\end{nota}
...
\begin{defn}[Riemann-Integral für Funktionen nach $\R^n$]
